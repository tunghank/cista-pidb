...
<%@ page import="com.cista.pidb.md.to.ZsHdcpKeyQueryTo" %>
...
<% 
	List<ZsHdcpKeyTo> result = (List<ZsHdcpKeyTo>)request.getAttribute("result");
	ZsHdcpKeyQueryTo criteria = (ZsHdcpKeyQueryTo)request.getAttribute("criteria");
%>
<html>
<head>
<!-- This is header -->
...
<script type="text/javascript">
	...
	function createWithRef() {
		if (!$('ref') || getRadioValue('ref') == null) {
			...
			document.location.href='<%=cp%>/md/zs_hdcpkey_create.do?m=pre&ref=' + getRadioValue('ref');
		}
	}
</script>
</head>
...
<table width="99%" border="0" cellpadding="0" cellspacing="0">
	<tbody>
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
			  <tr>
				<td class="pageTitle">Master Data :: HDCP KEY Query Result</td>
			  </tr>
			</table>
			<div class="content">
				...
				<tr>
					...
					<div align="right">
					  ...
					  <input name="button1" type="button" class="button" id="button1"value="Create New" 
					 	 onClick="document.location.href='<%=cp%>/md/zs_hdcp_key_create.jsp'">
					  ...
					  onClick="document.location.href='<%=cp%>/md/zs_hdcpkey_query.do?m=pre'">
					</div>
					</td>
				</tr>
			</table>
				...
				<tbody>
					<tr>
						<th width="20%">Material Number</th>
					  	...
					  	<td width="30%"><%=BeanHelper.getQueryCriteriaByColumn(criteria, "VENDOR_CODE") %>
					  	&nbsp; </td>
					</tr>
					<tr>
	...
	<form name="pagingForm" action="<%=cp %>/md/zs_hdcpkey_query.do" method="post">
...
%>
							<tr>
								<td <%=tdcss %>>
								<input type="radio" name="ref" value="<%=hdcpKey.getMaterialNum() %>">
								</td>
								<td <%=tdcss %>>
								<a href="<%=cp%>/md/zs_hdcpkey_edit.do?m=pre&materialNum=<%=hdcpKey.getMaterialNum() %>"><%=hdcpKey.getMaterialNum()%></a><%=BeanHelper.getHtmlValueByColumn(hdcpKey, "DESCRIPTION") %>
								&nbsp;</td>
								<td <%=tdcss %>>
								<%
					    		if ( BeanHelper.getHtmlValueByColumn(hdcpKey, "VENDOR_CODE") != null) {
					    			String listStr = BeanHelper.getHtmlValueByColumn(hdcpKey, "VENDOR_CODE");
							...
							<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(hdcpKey, "APPLICATION_PRODUCT") %>
								&nbsp;</td>
							<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(hdcpKey, "RELEASE_TO") %>
								&nbsp;</td>
							</tr>
<%
	}
