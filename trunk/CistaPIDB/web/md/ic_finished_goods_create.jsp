...
function doSave(s) {
	...
	new Ajax.Request(
		...
		{
			method: 'get',
			parameters: 'prodCode='+ $F('prodCode') + '&projCode=' + $F('projCode') + '&pkgCode=' + $F('pkgCode') + '&partNum=' + $F('partNum') + '&pkgType=' + $F('pkgType'),
...
function selectCP(projCode) {
	if ($F('cpMaterialNum')==""){
		alert ( "CP Material Number Must Choice" );
		return;
	}
...
function selectFT(partNum,projCode) {
	...
	var left = (screen.width-width)/2;
	var top = (screen.height-height)/2;
	var x = window.open(target,"select_ftProgram","scrollbar=auto, help=false, resizable=false, status=false,height="+height+"px,width="+width+"px,left="+left+"px,top="+top+"px");
	x.focus();
}
...
<form name="icFgForm" action="<%=cp %>/md/ic_fg_create.do?m=save" method="post">
	...
	<tbody>
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="pageTitle">Master Data :: Create IC Finished Goods</td>
				</tr>
			</table>
			<div class="content">
				...
				<tbody>
					...
					<tr>
						<th width="20%"><div><span class="star">*</span> Product Code</div></th>
					  	<td width="30%"><input class="text_required" type="text" readonly id="prodCode" name="prodCode"  value="<%=BeanHelper.getHtmlValueByColumn(icFgTo, "PROD_CODE") %>"  width="150px"> <img
							...
							SmartSearch.setup({
                                ...
                                name:"ProductCode",
                                autoSearch:false,
                                callbackHandle:"selectProdCallback"                                
							});
							function selectProdCallback(inputField, columns, value) {
					...
					<tr>
						<th width="20%"><div>Variant</div></th>
					  	<td width="30%"><input class="text_protected" type="text" readonly="true" id="variant" name="variant" value="" width="150px"></td>
					...
					<tr>
						...
						<td><input class="text_protected" type="text" readonly id="lfTool" name="lfTool" value=""  width="150px"></td>
					</tr>
					<tr>
						<th width="20%"><div class="erp_label"><span class="star">*</span> Package Code</div></th>
					  	<td width="30%"><input class="text_required" type="text" readonly id="pkgCode" name="pkgCode" value="<%=BeanHelper.getHtmlValueByColumn(icFgTo, "PKG_CODE") %>"  width="150px"> <img
							src="<%=cp%>/images/lov.gif" alt="LOV" onclick="selectPkgCode($('pkgType').value,$('prodCode').value,$('projCode').value)"></td>	
						<th>Close Lead Frame Name</th>
						<td><input class="text_200_protected" type="text" readonly id="closeLfName" name="closeLfName" value=""  width="150px"></td>
					</tr>
					<tr>
						<th width="20%"><div class="erp_label"><span class="star">*</span> Part Number</div></th>
					  	<td width="30%"><input class="text_200" type="text" id="partNum" name="partNum" value="<%=BeanHelper.getHtmlValueByColumn(icFgTo, "PART_NUM") %>"  width="150px" onKeyUp="setLFTool()"></td>			
					...
					<tr>
						<th width="20%"><div class="erp_label">Routing (AS)</div></th>
					  	<td width="30%"><input type="checkbox" id="routingAs" name="routingAs" width="150px" value="1"
							<%=icFgTo!=null && icFgTo.getRoutingAs()?"checked":"" %>>
						</td>				
						<th width="20%" rowspan="2"><div>AssignTo</div></th>
					  	...
					  	<select size="2" multiple class="text_two_line" name="assignTo" id="assignTo" >
					    	...
					    	<%
					    		if ( BeanHelper.getHtmlValueByColumn(icFgTo, "ASSIGN_TO") != null && BeanHelper.getHtmlValueByColumn(icFgTo, "ASSIGN_TO").length() > 0) {
					...
					<tr>
						...
						<th width="20%"><!--remove Hank 2008/01/29 <div class="erp_label">CP Material Number</div> --></th>
					  	<td width="30%">
						<!--remove Hank 2008/01/29 <select name="cpMaterialNum" id="cpMaterialNum" class="select_w130" onChange="selectCPTestProg()">
								<option value="">--Select--</option>
						</select> -->
						<!-- <input class="text_protected" type="text" readonly id="cpMaterialNum" name="cpMaterialNum" value="<%=BeanHelper.getHtmlValueByColumn(icFgTo, "CP_MATERIAL_NUM") %>" width="150px"> -->
					...
					<tr>
						...
						%>
					  	<td width="30%">
					  	<input type="hidden" id="cust" name="cust" value="<%=BeanHelper.getHtmlValueByColumn(sapMasterCustomerTo, "CUSTOMER_CODE")%>">
					  	<input class="text_protected" type="text" readonly="true" id="custName" name="custName" value="<%=BeanHelper.getHtmlValueByColumn(sapMasterCustomerTo, "SHORT_NAME")%>"  width="150px">
					  	</td>	
						<th width="20%" rowspan="4" style="vertical-align:top"><!--remove Hank 2008/01/29 <div class="erp_label">CP Test Program Name List</div> --></th>
					  	<td width="30%" rowspan="4" style="vertical-align:top">
						<!--remove Hank 2008/01/29 
					  	<input type="button" class="button" value="Add" onclick="selectCP($('projCode').value)"><input type="button" class="button" value="Remove" onclick="removeCp($('cpTestProgNameList'))"><br>
					...
					<tr>
						...
						<th width="20%" rowspan="4" style="vertical-align:top" ><!--remove Hank 2008/01/29 <div class="erp_label">FT Test Program List</div>--></th>
					  	<td width="30%" rowspan="4" style="vertical-align:top">
						<!--remove Hank 2008/01/29 
					  	<input type="button" class="button" value="Add" onclick="selectFT($('partNum').value,$('projCode').value)"><input type="button" class="button" value="Remove" onclick="removeSelectedOptions($('ftTestProgNameList'))"><br>
					...
					<tr>
						<th width="20%"><div>Status</div></th>
					  	<td width="30%"><input class="text_protected" type="text" readonly="true" id="status" name="status" value=""  width="150px"></td>		
					</tr>
					<tr>
