...
<%@ page import="com.cista.pidb.md.to.TradPkgQueryTo" %>
...
<script type="text/javascript">
	...
	function createWithRef() {
		if (!$('ref') || getRadioValue('ref') == null) {
			...
			document.location.href='<%=cp%>/md/trad_pkg_create.do?m=pre&ref=' + getRadioValue('ref');
		}
	}
</script>
</head>
<body>
<% 	List<TradPkgTo> result = (List<TradPkgTo>)request.getAttribute("result");
	TradPkgQueryTo criteria = (TradPkgQueryTo)request.getAttribute("criteria");
%>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="pageTitle">Master Data :: Traditional Package Query Result</td>
  </tr>
</table>
			<div class="content">
				...
				<tr>
					...
					<div align="right">
					  ...
					  <input name="button1" type="button" class="button" id="button1"value="Create New" 
					 	 onClick="document.location.href='<%=cp%>/md/trad_pkg_create.jsp'">
					  ...
					  onClick="document.location.href='<%=cp%>/md/trad_pkg_query.jsp'">
					</div>
					</td>
				</tr>
			</table>
	...
	<form name="pagingForm" action="<%=cp %>/md/trad_pkg_query.do" method="post">
...
%>
							<tr>
								...
								<td <%=tdcss %>>
								<a href="<%=cp%>/md/trad_pkg_edit.do?m=pre&pkgName=<%=BeanHelper.getHtmlValueByColumn(oneIc, "PKG_NAME") %>"><%=BeanHelper.getHtmlValueByColumn(oneIc, "PROJ_NAME")%></a>&nbsp;</td>
