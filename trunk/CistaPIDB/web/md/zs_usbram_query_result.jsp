...
<%@ page import="com.cista.pidb.md.to.ZsUsbramQueryTo" %>
...
<% 
	List<ZsUsbramTo> result = (List<ZsUsbramTo>)request.getAttribute("result");
	ZsUsbramQueryTo criteria = (ZsUsbramQueryTo)request.getAttribute("criteria");
%>
<html>
<head>
<!-- This is header -->
...
<script type="text/javascript">
	...
	function createWithRef() {
		if (!$('ref') || getRadioValue('ref') == null) {
			...
			document.location.href='<%=cp%>/md/zs_usbram_create.do?m=pre&ref=' + getRadioValue('ref');
		}
	}
</script>
</head>
...
<table width="99%" border="0" cellpadding="0" cellspacing="0">
	<tbody>
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
			  <tr>
				<td class="pageTitle">Master Data :: USBRAM Query Result</td>
			  </tr>
			</table>
			<div class="content">
				...
				<tr>
					...
					<div align="right">
					  ...
					  <input name="button1" type="button" class="button" id="button1"value="Create New" 
					 	 onClick="document.location.href='<%=cp%>/md/zs_usbram_create.jsp'">
					  ...
					  onClick="document.location.href='<%=cp%>/md/zs_usbram_query.do?m=pre'">
					</div>
					</td>
				</tr>
			</table>
			<table class="formFull" id="queryCriteriaPanel" border="0" cellpadding="1" cellspacing="1">
				<tbody>
					...
					<tr>
						<th width="20%">Input/Output</th>
					  	<td width="30%"><%=BeanHelper.getQueryCriteriaByColumn(criteria, "IO") %>
					  	&nbsp; </td>
					...
					<tr>
						...
						<th width="20%">Support Max Resolution</th>
					  	<td width="30%"><%=BeanHelper.getQueryCriteriaByColumn(criteria, "MAX_RESOLUTION") %>
						</td>
					</tr>
					<tr>
	...
	<form name="pagingForm" action="<%=cp %>/md/zs_usbram_query.do" method="post">
					...
					<table class="grid" border="0" cellpadding="1" cellspacing="1">
						<tbody>
							<tr>
								...
								<th onclick="sort(this, 1)" style="cursor:pointer">IO</th>
								<th onclick="sort(this, 1)" style="cursor:pointer">Support Max Resolution</th>
								<th onclick="sort(this, 1)" style="cursor:pointer">APPLICATION PRODUCT</th>
								<th onclick="sort(this, 1)" style="cursor:pointer">COMPANY</th>
							</tr>
<%
	int idx = 0;
	for(ZsUsbramTo usbram : result) {
		idx ++;
		String tdcss = "class=\"c" + idx % 2+"\"";		
%>
							<tr>
								<td <%=tdcss %>>
								<input type="radio" name="ref" value="<%=usbram.getMaterialNum() %>">
								</td>
								<td <%=tdcss %>>
								<a href="<%=cp%>/md/zs_usbram_edit.do?m=pre&materialNum=<%=usbram.getMaterialNum() %>"><%=usbram.getMaterialNum()%></a>&nbsp;</td>
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(usbram, "DESCRIPTION") %>
								&nbsp;</td>
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(usbram, "MEMORY_SIZE") %>
								&nbsp;</td>
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(usbram, "OPERATION_VOLTAGE") %>
								&nbsp;</td>
								<td <%=tdcss %>>
								<%
					    		if ( BeanHelper.getHtmlValueByColumn(usbram, "VENDOR_CODE") != null) {
					    			String listStr = BeanHelper.getHtmlValueByColumn(usbram, "VENDOR_CODE");
					    	...
					    	%>	
								...
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(usbram, "CREATED_BY") %>
								&nbsp;</td>
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(usbram, "MODIFIED_BY") %>
								&nbsp;</td>	
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(usbram, "IO") %>
								&nbsp;</td>
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(usbram, "MAX_RESOLUTION") %>
								&nbsp;</td>	
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(usbram, "APPLICATION_PRODUCT") %>
								&nbsp;</td>
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(usbram, "RELEASE_TO") %>
								&nbsp;</td>
							</tr>
<%
	}
