...
function doSave() {
		...
		new Ajax.Request(
			'<%=cp%>/ajax/check_csp_exist.do',
			{
				method: 'get',
				parameters: 'pkgName='+ $F('pkgName'),
				onComplete: checkCspExistComplete
			}
		);
	}	
	
 function checkCspExistComplete(r) {
	var result = r.responseText;
	if(result=="true") {
		setMessage("error", "The CSP package is already exist.");
		$('saveBtn').disabled = false;
		$('resetBtn').disabled = false;
		$('isRelease').value='0';	
	} else {
		
			setMessage("error", "Saving CSP package...");		
...
<body>
<%
	CspTo ref = (CspTo) request.getAttribute("ref");
%>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
		<td class="pageTitle">Master Data :: Create CSP Package</td>
  </tr>
</table>
			<div class="content">
			<form id="createForm" action="<%=cp%>/md/csp_create.do" method="post">
			...
			<table class="formFull" border="0" cellpadding="1" cellspacing="1">
				<tbody>
					...
					<tr>
						...
						<td><input class="text" type="text" name="cspType" id="cspType"
