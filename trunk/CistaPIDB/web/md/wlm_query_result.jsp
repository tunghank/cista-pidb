...
<%@ page import="com.cista.pidb.md.to.WlmQueryTo" %>
...
<script type="text/javascript">
	...
	function createWithRef() {
		if (!$('ref') || getRadioValue('ref') == null) {
			...
			document.location.href='<%=cp%>/md/wlm_create.do?m=pre&ref=' + getRadioValue('ref');
		}
	}
</script>
</head>
<body>
<% 	List<WlmTo> result = (List<WlmTo>)request.getAttribute("result");
	WlmQueryTo criteria = (WlmQueryTo)request.getAttribute("criteria");
%>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="pageTitle">Master Data :: WLM Material Query Result</td>
  </tr>
</table>
			<div class="content">
				...
				<tr>
					...
					<div align="right">
					  ...
					  <input name="button1" type="button" class="button" id="button1"value="Create New" 
					 	 onClick="document.location.href='<%=cp%>/md/wlm_create.do?m=pre'">
					  ...
					  onClick="document.location.href='<%=cp%>/md/wlm_query.do?m=pre'">
					</div>
					</td>
				</tr>
			</table>
				...
				<tbody>
					<tr>
					    ...
					    &nbsp;</td>
					  	...
					  	&nbsp;</td>
						<th width="10%">Package Code</th>
					  	<td width="20%"><%=BeanHelper.getQueryCriteriaByColumn(criteria, "PACKAGE_CODE") %>
					  	&nbsp;</td>
					</tr>
					<tr>
	...
	<form name="pagingForm" action="<%=cp %>/md/wlm_query.do" method="post">
...
%>
							<tr>
								...
								<td <%=tdcss %>>
								<a href="<%=cp%>/md/wlm_edit.do?m=pre&materialNum=<%=BeanHelper.getHtmlValueByColumn(oneIc, "MATERIAL_NUM") %>"><%=BeanHelper.getHtmlValueByColumn(oneIc, "MATERIAL_NUM")%></a>&nbsp;</td>
								...
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(oneIc, "PACKAGE_CODE") %>&nbsp;</td>
							</tr>
<%
	}
%>
