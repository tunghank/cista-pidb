...
function doSave(s) {
	...
	setMessage("error", "Checking DESCRIPTION exist...");
	new Ajax.Request(
		'<%=cp%>/ajax/check_Hdcpkey_exist.do',
		{
			method: 'get',
			parameters: 'materialNum='+ $F('materialNum') + '&description=' + $F('description'),
			onComplete: checkHdcpKeyExistComplete
		}
	);
}

function checkHdcpKeyExistComplete(r) {
	...
	if(result == "materialNum") {
		...
		if (incompleted != "") {
			if (confirm("The following required fields is incomplete, the HDCP_KEY will be saved as draft:\r\n" + incompleted.substring(2))) {
				setMessage("error", "Saving HDCP_KEY...");
				selectAllOptions($('hdcpKeyVendor'));
				selectAllOptions($('prodCodeList'));
				$('zsHdcpKeyForm').action = $F('description').action + "&toErp=" + $F('toErp');
				$('zsHdcpKeyForm').submit();
			} else {
				$('releaseBtn').disabled = false;
				$('saveBtn').disabled = false;
				$('resetBtn').disabled = false;
			...
			setMessage("error", "Saving HDCP KEY...");
			selectAllOptions($('hdcpKeyVendor'));
			selectAllOptions($('prodCodeList'));
			$('zsHdcpKeyForm').action = $('zsHdcpKeyForm').action + "&toErp=" + $F('toErp');
			$('zsHdcpKeyForm').submit();
		}
	}
}
...
<form name="zsHdcpKeyForm" action="<%=cp %>/md/zs_hdcpkey_create.do?m=save" method="post">
