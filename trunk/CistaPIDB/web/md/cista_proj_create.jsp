...
function doRelease() {
	...
	setMessage("error", "Checking project exist...");

	new Ajax.Request(
		'<%=cp%>/ajax/check_proj_exist.do',
		{
			method: 'get',
			parameters: 'projName='+ $F('projName') + '&projOption=' + $F('projOption'),
			onComplete: checkProjExistComplete2
		}
	);
}

function checkProjExistComplete2(r) {
	var result = r.responseText;
	if(result=="true") {
		setMessage("error", "Project code is already exist.");
...
function FetchProjCodesComplete(r) {
    ...
    if (confirm("There are project codes:" + $F('projName') + "-" + $F('projOption') + "," + codes + " with this project name, release all?")) {
        act += "&releaseType=all";
    } else {
        act += "&releaseType=single"
    }
    $('projectCreate').action += act;
    setMessage("error", "Releasing project...");
    $('projectCreate').submit();
}

function doSave() {
    if (!checkRequiredForSave()) return;
	...
	new Ajax.Request(
		'<%=cp%>/ajax/check_proj_exist.do',
		{
			method: 'get',
			parameters: 'projName='+ $F('projName') + '&projOption=' + $F('projOption'),
			onComplete: checkProjExistComplete
		}
	);
}

function checkProjExistComplete(r) {
	var result = r.responseText;
	if(result=="true") {
		setMessage("error", "Project code is already exist.");
		$('releaseBtn').disabled = false;
...
function checkRequired() {
	...
	if ($F('releaseTo') == "" || $F('releaseTo').length == 0) {
		setMessage("error", "For WP must be choice.");
		return;
	}
...
function retrieveProj() {
    if ($F('projName').length != 6) return;
	new Ajax.Request(
		'<%=cp%>/ajax/fetch_proj.do',
		{
			method: 'post',
			parameters: 'projName='+ $F('projName'),
			onComplete: retrieveProjComplete
		}
	);
}

function retrieveProjComplete(r) {
    var xmlObj = r.responseXML;
    var projs = xmlObj.getElementsByTagName("ProjectTo");
    //if not found
    ...
    var nodes = projs[0].childNodes;
    for (var i = 0; i < nodes.length; i++) {
        var field = $(nodes[i].tagName);
        if (nodes[i].tagName == "projCode") continue;
        if (nodes[i].tagName == "status") continue;
        if (nodes[i].tagName == "projOption") continue;
        if (nodes[i].tagName == "createdBy") field = $("createBy");
        
        //if prodCodeList
        if (nodes[i].tagName == "prodCodeList") {
            var values = nodes[i].firstChild.nodeValue.split(",");
            var opts = $('prodCodeList').options;
            //remove all options
            opts.length = 0;
            for (var j = 0; j < values.length; j++) {
                if (values[j] == "") break;
                opts[j] = new Option(values[j], values[j]);
            }
        }
...
<body>
<form name="projectCreate" action="<%=cp %>/md/proj_create.do?m=save" method="post">
	...
	<tbody>
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="pageTitle">Master Data ::Cista Create Project</td>
				</tr>
			</table>
			<div class="content">
			...
			<table class="formFull" border="0" cellpadding="1" cellspacing="1">
				<tbody>
					<tr>
						...
						<td width="22%"><input type="text" class="text_protected" name="projCode" id="projCode" value="" readonly></td>
						<th width="11%"><div class="erp_label">Wafer Gross</div></th>
						<td width="22%"><input type="text" class="text" name="grossDie" id="grossDie" value="<%=BeanHelper.getHtmlValueByColumn(ref, "GROSS_DIE") %>">
						 &nbsp;&nbsp;&nbsp;Est. <input type="checkbox" id="estimated" name="estimated" value="1" <%=ref!=null && ref.getEstimated()?"checked":"" %>>
						</td>	
					...
					<tr>
						<th width="11%"><div class="erp_label"><span class="star">*</span>Project Name</div></th>
						<td width="22%"><input type="text" class="text_required" name="projName" id="projName" value="" onblur="retrieveProj()"></td>
					...
					<tr>
						...
						<td width="22%">
							<select style="width:180px" name="fab" id="fab" onChange='selFabVendorCode()'>
								<option value="">--Select--</option>
								<%
									...
									for (FabCodeTo fabCode : fabCodeList) {
										...
										String selected = "";
										if (v.equals(BeanHelper.getHtmlValueByColumn(ref, "FAB"))) {
											selected = "selected";
										}
										%>
								<option value="<%=fabCode.getFab()%>" <%=selected %>><%=v %></option>
										<%
									}
								%>
							</select>
					    ...
					    <th width="11%">PM</th>
					    <td width="23%"><input type="text" class="text" name="pm" id="pm" value="<%=BeanHelper.getHtmlValueByColumn(ref, "PM") %>"></td>
					</tr>
					...
					<tr>
					    ...
					    <td width="23%"><input type="text" class="text" name="qaEngr" id="qaEngr" value="<%=BeanHelper.getHtmlValueByColumn(ref, "QA_ENGR") %>"></td>
					</tr>

					<tr>
						<th width="11%">Option</th>
						<td width="22%"><input type="text" class="text" name="projOption" id="projOption" value="<%=BeanHelper.getHtmlValueByColumn(ref, "PROJ_OPTION") %>" maxlength="2"></td>
					...
					<tr>
					    ...
					    <table border="0" cellpadding="0" cellspacing="0"><tr><td rowspan="2">
					...
					<tr>
						...
						<td width="22%"><input type="text" class="text" name="procName" id="procName" value="<%=BeanHelper.getHtmlValueByColumn(ref, "PROC_NAME") %>"></td>
					...
					<tr>
						...
						<td width="22%"><input type="text" class="text" name="nickName" id="nickName" value="<%=BeanHelper.getHtmlValueByColumn(ref, "NICK_NAME") %>">
						</td>
						<th width="11%">Any IP Usage</th>
						<td width="22%">						<select class="select_w130" name="anyIpUsage" id="anyIpUsage" value="">
							<option value="">--Select--</option>
					    ...
					    <td width="23%"><input class="text" readonly name="prodLine" id="prodLine" value="<%=BeanHelper.getHtmlValueByColumn(ref, "PROD_LINE") %>"> 
					      <img
					...
					<tr>
						...
						<td width="22%">						<select class="select_w130" name="embeddedOtp" id="embeddedOtp">
							<option value="">--Select--</option>
					...
					<tr>
						<th width="11%"><div class="erp_label">Pitch (UM)</div></th>
						<td width="22%"><input type="text" class="text" name="pitch" id="pitch" value="<%=BeanHelper.getHtmlValueByColumn(ref, "PITCH") %>"></td>								
						<th width="11%">OTP Size</th>
						<td width="22%"><input type="text" class="text" name="otpSize" id="otpSize" value="<%=BeanHelper.getHtmlValueByColumn(ref, "OTP_SIZE") %>">
						</td>
					...
					<tr>
						...
						<img
							...
							height="20" id="kickoffdateBtn"> <script
					...
					<tr>
						...
						<td width="22%"><input type="text" class="text" name="fsDate" id="fsDate" value="<%=BeanHelper.getHtmlValueByColumn(ref, "FS_DATE", "yyyy/MM/dd") %>" readonly>
							<img
							src="<%=cp%>/images/calendar.gif" alt="Calendar" width="19"
							height="20" id="fsDateBtn" style="vertical-align:middle">
							<script type="text/javascript">
							Calendar.setup({
								inputField:"fsDate",
								ifFormat:"%Y/%m/%d",
						...
						<td width="22%"><input type="text" class="text" name="projLeader" id="projLeader" value="<%=BeanHelper.getHtmlValueByColumn(ref, "PROJ_LEADER") %>">
						</td>
					    <th width="11%">Remark</th>
					    <td width="23%"><input type="text" class="text" name="remark" id="remark" value="<%=BeanHelper.getHtmlValueByColumn(ref, "REMARK") %>">
					    </td>
					</tr>
					<tr>
						<th width="11%"><div class="erp_label">Process Technology</div></th>
						<td width="22%">
						...
						<td width="22%"><input type="text" class="text" name="designEngr" id="designEngr" value="<%=BeanHelper.getHtmlValueByColumn(ref, "DESIGN_ENGR") %>">
						</td>
					...
					<tr>
						...
						<td width="22%"><input type="text" class="text" name="prodEngr" id="prodEngr" value="<%=BeanHelper.getHtmlValueByColumn(ref, "PROD_ENGR") %>">
						</td>
					...
					<tr>
						...
						<td width="22%"><input type="text" cols="80%" readonly name="voltage" id="voltage" value="<%=BeanHelper.getHtmlValueByColumn(ref, "VOLTAGE") %>" class="text"> 
					      <img
							src="<%=cp%>/images/lov.gif" alt="LOV" id="voltageSSBtn">
							<script type="text/javascript">
							SmartSearch.setup({
                                cp:"<%=cp%>",
                                button:"voltageSSBtn",
						...
						<td width="22%"><input type="text" class="text" name="esdEngr" id="esdEngr" value="<%=BeanHelper.getHtmlValueByColumn(ref, "ESD_ENGR") %>">
						</td>
					</tr>
					...
					<tr>
						...
						<td width="22%"><input type="text" class="text" name="aprEngr" id="aprEngr" value="<%=BeanHelper.getHtmlValueByColumn(ref, "APR_ENGR") %>"></td>
					</tr>
					...
					<tr>
					  ...
					  <td width="22%"><input type="text" class="text" name="layoutEngr" id="layoutEngr" value="<%=BeanHelper.getHtmlValueByColumn(ref, "LAYOUT_ENGR") %>">
					  </td>
					</tr>
					...
					<tr>
						...
						<td width="22%"><input type="text" class="text" name="testEngr" id="testEngr" value="<%=BeanHelper.getHtmlValueByColumn(ref, "TEST_ENGR") %>">
						</td>
				    </tr>
					...
					<tr>
						...
						<td width="22%"><input type="text" class="text" name="assyEngr" id="assyEngr" value="<%=BeanHelper.getHtmlValueByColumn(ref, "ASSY_ENGR") %>">
						</td>
				    </tr>

					<tr>
						<th width="11%">X Size (UM;含SR)</th>
						<td width="22%"><input type="text" class="text" name="xSize" id="xSize" value="<%=BeanHelper.getHtmlValueByColumn(ref, "X_SIZE") %>"></td>										
						<th width="11%"><div class="erp_label">2nd Foundry Project</div></th>
						<td width="22%"><select class="select_w130" name="secondFoundryProject" id="secondFoundryProject">
							<option value="">--Select--</option>
							<option value="Yes" <%="Yes".equals(BeanHelper.getHtmlValueByColumn(ref, "SECOND_FOUNDRY_PROJECT"))?"selected":"" %>>Yes</option>
							<option value="No" <%="No".equals(BeanHelper.getHtmlValueByColumn(ref, "SECOND_FOUNDRY_PROJECT"))?"selected":"" %>>No</option></select>
							</td>
				    ...
				    <tr>
						...
						<td width="22%"><input type="text" class="text" name="ySize" id="ySize" value="<%=BeanHelper.getHtmlValueByColumn(ref, "Y_SIZE") %>"></td>				    
					...
					<div align="right">
						...
						if ( isGuest.equals("No"))  {
						   %>
						     ...
						     onclick="doRelease()"> <input name="saveBtn" type="button" class="button" id="saveBtn"
