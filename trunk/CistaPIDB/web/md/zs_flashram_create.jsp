...
<%
	ZsFlashramTo ref = (ZsFlashramTo) request.getAttribute("ref");
...
function doSave(s) {
	...
	setMessage("error", "Checking ZS_FLASHRAM exist...");
	new Ajax.Request(
		'<%=cp%>/ajax/check_zsFlashram_exist.do',
		{
			method: 'get',
			parameters: 'materialNum='+ $F('materialNum') + '&description=' + $F('description'),
			onComplete: checkZsFlashramExistComplete
		}
	);
}

function checkZsFlashramExistComplete(r) {
	...
	if(result == "materialNum") {
		...
		if (incompleted != "") {
			if (confirm("The following required fields is incomplete, the FLASHRAM will be saved as draft:\r\n" + incompleted.substring(2))) {
				setMessage("error", "Saving FLASHRAM...");
				selectAllOptions($('flashramVendor'));
				selectAllOptions($('prodCodeList'));
				$('zsFlashramForm').action = $('zsFlashramForm').action + "&toErp=" + $F('toErp');
				$('zsFlashramForm').submit();
			} else {
				$('releaseBtn').disabled = false;
				$('saveBtn').disabled = false;
				$('resetBtn').disabled = false;
			...
			setMessage("error", "Saving FLASH-RAM...");
			selectAllOptions($('flashramVendor'));
			 selectAllOptions($('prodCodeList'));
			$('zsFlashramForm').action = $('zsFlashramForm').action + "&toErp=" + $F('toErp');
			$('zsFlashramForm').submit();
...
function selectVendorComplete(selectedTests) {
	if (selectedTests && selectedTests.length>0) {
		for(var i=0; i<selectedTests.length; i++) {
			...
			addOption($('flashramVendor'), test[1], test[0]);
		}
	}
}
...
<form name="zsFlashramForm" action="<%=cp %>/md/zs_flashram_create.do?m=save" method="post">
	...
	<tbody>
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="pageTitle">Master Data :: Create ZS_FLASHRAM</td>
				</tr>
			</table>
			<div class="content">
			...
			<table class="formFull" border="0" cellpadding="1" cellspacing="1">
				<tbody>
					...
					<tr>
						<th width="20%" rowspan="2" style="vertical-align:top"><div>FLASHRAM Vendor</div></th>
					  	<td width="30%" rowspan="2" style="vertical-align:top">
					  	<input type="button" class="button" value="Add" onclick="selectVendor()"><input type="button" class="button" value="Remove" onclick="removeSelectedOptions($('flashramVendor'))"><br>
					  	<select size="4" multiple style="width:50%" name="flashramVendor" id="flashramVendor">
					    	<%
