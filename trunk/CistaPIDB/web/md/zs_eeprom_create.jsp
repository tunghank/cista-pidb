...
function doSave(s) {
	...
	setMessage("error", "Checking ZS_EEPROM exist...");
	new Ajax.Request(
		'<%=cp%>/ajax/check_zsEeprom_exist.do',
		{
			method: 'get',
			parameters: 'materialNum='+ $F('materialNum') + '&Code_Bind=' + $F('codeBind'),
			onComplete: checkZsEepromExistComplete
		}
	);
}

function checkZsEepromExistComplete(r) {
	...
	if(result == "materialNum") {
	...
	} else if(result == "codeBind") {
		setMessage("error", "Code_Bind is error.");
		$('releaseBtn').disabled = false;
		$('saveBtn').disabled = false;
		$('resetBtn').disabled = false;
	}else {
		...
		if (incompleted != "") {
			if (confirm("The following required fields is incomplete, the EEPROM will be saved as draft:\r\n" + incompleted.substring(2))) {
				setMessage("error", "Saving EEPROM...");
				selectAllOptions($('eepromVendor'));
				selectAllOptions($('prodCodeList'));
				$('zsEepromForm').action = $('zsEepromForm').action + "&toErp=" + $F('toErp');
				$('zsEepromForm').submit();
			} else {
				$('releaseBtn').disabled = false;
				$('saveBtn').disabled = false;
...
<form name="ZsEepromForm" action="<%=cp %>/md/zs_eeprom_create.do?m=save" method="post">
					 	<input type="hidden" id="assignEmail" name="assignEmail" 
					 	value="<%=BeanHelper.getHtmlValueByColumn(ref,"ASSIGN_EMAIL") %>"><input type="hidden" id="createdBy" name="createdBy" 
	...
	<tbody>
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="pageTitle">Master Data :: Create EEPROM</td>
				</tr>
			</table>
			<div class="content">
