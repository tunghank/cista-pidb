...
<%@ page import="com.cista.pidb.md.to.ZsSdramQueryTo" %>
<%@ page import="com.cista.pidb.md.to.ZsSdramTo" %>
...
<% 
	List<ZsSdramTo> result = (List<ZsSdramTo>)request.getAttribute("result");
	ZsSdramQueryTo criteria = (ZsSdramQueryTo)request.getAttribute("criteria");
%>
<html>
<head>
<!-- This is header -->
...
<script type="text/javascript">
	...
	function createWithRef() {
		if (!$('ref') || getRadioValue('ref') == null) {
			...
			document.location.href='<%=cp%>/md/zs_sdram_create.do?m=pre&ref=' + getRadioValue('ref');
		}
	}
</script>
</head>
...
<table width="99%" border="0" cellpadding="0" cellspacing="0">
	<tbody>
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
			  <tr>
				<td class="pageTitle">Master Data :: SDRAM Query Result</td>
			  </tr>
			</table>
			<div class="content">
				...
				<tr>
					...
					<div align="right">
					  ...
					  <input name="button1" type="button" class="button" id="button1"value="Create New" 
					 	 onClick="document.location.href='<%=cp%>/md/zs_sdram_create.jsp'">
					  ...
					  onClick="document.location.href='<%=cp%>/md/zs_sdram_query.do?m=pre'">
					</div>
					</td>
				</tr>
			</table>
	...
	<form name="pagingForm" action="<%=cp %>/md/zs_sdram_query.do" method="post">
...
%>
							<tr>
								<td <%=tdcss %>>
								<input type="radio" name="ref" value="<%=sdram.getMaterialNum() %>">
								</td>
								<td <%=tdcss %>>
								<a href="<%=cp%>/md/zs_sdram_edit.do?m=pre&materialNum=<%=sdram.getMaterialNum() %>"><%=sdram.getMaterialNum()%></a>&nbsp;</td>
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(sdram, "DESCRIPTION") %>
								&nbsp;</td>
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(sdram, "MEMORY_SIZE") %>
								&nbsp;</td>
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(sdram, "SPEED") %>
								&nbsp;</td>
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(sdram, "OPERATION_VOLTAGE") %>
								&nbsp;</td>
								<td <%=tdcss %>>
								<%
					    		if ( BeanHelper.getHtmlValueByColumn(sdram, "VENDOR_CODE") != null) {
					    			String listStr = BeanHelper.getHtmlValueByColumn(sdram, "VENDOR_CODE");
					    	...
					    	%>	
								...
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(sdram, "CREATED_BY") %>
								&nbsp;</td>
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(sdram, "MODIFIED_BY") %>
								&nbsp;</td>							
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(sdram, "APPLICATION_PRODUCT") %>
								&nbsp;</td>
								<td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(sdram, "RELEASE_TO") %>
								&nbsp;</td>
							</tr>
<%
	}
