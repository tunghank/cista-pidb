...
<script type="text/javascript">
	function doSave() {
		...
		new Ajax.Request(
			'<%=cp%>/ajax/check_esdTest_exist.do',
			{
				method: 'get',
				parameters: 'projCodeWVersion='+ $F('projCodeWVersion')+'&idEsdTesting='+$F('idEsdTesting'),
				onComplete: checkEsdTEstExistComplete
			}
		);
	}
	
function checkEsdTEstExistComplete(r) {
	var result = r.responseText;
	if(result=="true") {
		setMessage("error", "The esd test is already exist.");
		$('saveBtn').disabled = false;
		$('resetBtn').disabled = false;
	} else {		
		setMessage("error", "Saving esd test...");	
		selectAllOptions($('assignTo'));
											
		$('createForm').action = $('createForm').action;
		$('createForm').submit();
	}
}
	...
	<tbody>
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
			  <tr>
				<td class="pageTitle">Master Data :: Create ESD Test </td>
			  </tr>
			</table>
			<div class="content">
			<form id="createForm" action="<%=cp%>/md/esd_test_create.do?m=save" method="post">
			...
			<table class="formFull" border="0" cellpadding="1" cellspacing="1">
				<tbody>
					...
					<th width="20%"><span class="star">*</span> Product Code</th>
					  <td width="30%">
					  	<input type="text" class="text" name="prodCode" id="prodCode" value="<%=BeanHelper.getHtmlValueByColumn(ref,"PROD_CODE") %>" readonly>
					   ...
					   <th rowspan="2">Results--HBM</th>
						<td rowspan="2"><textarea class="textarea" type="text" cols="42" rows="2" name="resultsHbm" id="resultsHbm"><%=BeanHelper.getHtmlValueByColumn(ref,"RESULTS_HBM") %></textarea>
						&nbsp;
					...
					<th width="20%"><span class="star">*</span> Project Code w Version</th>
					  	<td width="30%">
					  	<input type="text" class="text" name="projCodeWVersion" id="projCodeWVersion"  readonly>
					...
					<tr>
					  ...
					  <td rowspan="2"><textarea class="textarea" type="text" cols="42" rows="2" name="resultsMm" id="resultsMm" ><%=BeanHelper.getHtmlValueByColumn(ref,"RESULTS_MM") %></textarea>
					  	&nbsp;
					  	<select  class="select_w80" name="resultsMmPf" id="resultsMmPf">
					      <option value="">--Select--</option>
					      <option value="PASS" <%="PASS".equals(BeanHelper.getHtmlValueByColumn(ref, "RESULTS_MM_SELECT"))?"selected":"" %> >PASS</option> 
					      <option value="FAIL" <%="FAIL".equals(BeanHelper.getHtmlValueByColumn(ref, "RESULTS_MM_SELECT"))?"selected":"" %>>FAIL</option>
				        </select>
					  </td>
					</tr>
					<tr>
					...
					<tr>
					  ...
					  <td rowspan="2"><textarea class="textarea" type="text" cols="42" rows="2" name="resultsCdm" id="resultsCdm"> <%=BeanHelper.getHtmlValueByColumn(ref,"RESULTS_CDM") %></textarea>
					  	&nbsp;
					 ...
					 <tr>
					    ...
					    <td rowspan="2"><textarea class="textarea" type="text" cols="42" rows="2" name="resultsLu" id="resultsLu"> <%=BeanHelper.getHtmlValueByColumn(ref,"RESULTS_LU") %></textarea>
					  	&nbsp;
