...
function doSave(s) {
	...
	setMessage("error", "Checking ft test program exist...");
	new Ajax.Request(
		'<%=cp%>/ajax/check_ftTestProgram_exist.do',
		{
			method: 'get',
			parameters: 'ftTestProgName='+ $F('ftTestProgName')+ '&ftMaterialNum=' + $F('ftMaterialNum'),
			onComplete: checkFtTestProgramExistComplete
		}
	);
}

function checkFtTestProgramExistComplete(r) {
	var result = r.responseText;
	if(result=="true") {
		setMessage("error", "FT Test Program Name is already exist.");
		...
		$('resetBtn').disabled = false;
			...
			$('ftTestProgCreate').action = $('ftTestProgCreate').action + "?m=save&toErp=" + $F('toErp');
			$('ftTestProgCreate').submit();
	}
}
...
function createWithReference(){
	var partNum = $('partNum').value;
		if($('partNum') != ""){
			findMaterialName(partNum);
		}
}

function findMaterialName(value){
	 if (value != null && value.length > 0) {
	   removeOptions($('ftMaterialNum'));
		...
		'<%=cp%>/ajax/fetch_icFgMaterialNum.do',
		{
			...
			onComplete: fetchIcFtMaterialNumComplete
		} );
...
<form name="ftTestProgCreate"
	action="<%=cp %>/md/ft_test_prog_create.do" method="post">
	...
	<tbody>
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="pageTitle">Master Data :: Create FT Test Program</td>
				</tr>
			</table>
			<div class="content">
			...
			<table class="formFull" border="0" cellpadding="1" cellspacing="1">
				<tbody>
				    <tr>
						...
						<td>
						<select name="ftMaterialNum" id="ftMaterialNum" class="select_w150" >
					...
					<tr>
						...
						<td><input class="text_required" name="partNum"
							id="partNum" readonly
							value="<%=BeanHelper.getHtmlValueByColumn(ref, "PART_NUM") %>">
						<img src="<%=cp%>/images/lov.gif" alt="LOV" id="partNumSSBtn">
							...
							function fetchIcFtMaterialNumComplete(r) {
								var returnValue = r.responseText.split("|");
								addOption($('ftMaterialNum'), "--Select--", "");
								if (returnValue.length > 0) {
									
									for (i = 0 ; i < returnValue.length; i++) {
										if ( returnValue[i].length > 1 ) {
											addOption($('ftMaterialNum'), returnValue[i], returnValue[i]);
										}
									}
									
									if ( $('ftMaterialNum').length == 2 ){
										removeOption($('ftMaterialNum'),0);
									}else if ( $('ftMaterialNum').length > 2 ) {
										
									}else{
										removeOption($('ftMaterialNum'));
									}
								}
							}
							</script>							</td>
					...
					<tr>
						...
						<td><input class="text_required" name="ftTestProgName"
							id="ftTestProgName"></td>
					</tr>
					<tr>
						<th><span class="star">*</span>FT Test Program Revision</th>
						<td><input class="text_required" maxlength="20" size="20"
					...
					<tr>
						...
						<td><input name="ftTestProgReleaseDate" type="text"
							class="text" id="ftTestProgReleaseDate" readonly
