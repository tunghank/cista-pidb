...
function doSave() {
		...
		new Ajax.Request(
			'<%=cp%>/ajax/check_wlh_exist.do',
			{
				method: 'get',
				parameters: 'materialNum='+ $F('materialNum'),
				onComplete: checkWlhExistComplete
			}
		);
	}	
	
 function checkWlhExistComplete(r) {
	var result = r.responseText;
	if(result=="true") {
		setMessage("error", "The WLH Material is already exist.");
		...
		$('isRelease').value='0';	
			...
			setMessage("error", "Saving WLH Material package...");		
...
function fetchWlhMaterialNum() {
	new Ajax.Request(
			'<%=cp%>/ajax/fetch_WlhMaterialNum.do',
			{
				method: 'post',
				...
				onComplete: fetchWlhMaterialNumComplete
			}
		);
}

function fetchWlhMaterialNumComplete(r) {
	var result = r.responseText;
	var rt  = result.split("|");
	var material = rt[0];
	var partNum = rt[1];
	...
	if (result == "") {
	  	setMessage("error", "Can not fetch WLH Material Number.");		  	
	} else {
...
<body>
<%
	WlhTo ref = (WlhTo) request.getAttribute("ref");
%>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
		<td class="pageTitle">Master Data :: Create WLH Material</td>
  </tr>
</table>
			<div class="content">
			<form id="createForm" action="<%=cp%>/md/wlh_create.do" method="post">
			...
			<table class="formFull" border="0" cellpadding="1" cellspacing="1">
				<tbody>
					...
					<tr>
					    ...
					    <td width="30%">
						  <select class="select_w130" name="packingType" id="packingType" onChange="fetchWlhMaterialNum()">
					...
					<tr>
					    ...
					    <td width="30%"><select class="select_w130" name="prodType" id="prodType" onChange="fetchWlhMaterialNum()">
								<option value="">--Select--</option>
								<%
					...
					<tr>
					    ...
					    <td width="30%"><select class="select_w130" name="vendorCode" id="vendorCode" onChange="fetchWlhMaterialNum()">
								<option value="">--Select--</option>
								<%
					...
					<tr>
						<th width="20%"><div>Vendor Device</div></th>
					  	<td width="30%"><input class="text" type="text" id="vendorDevice" name="vendorDevice" value=""  width="150px"></td>
						<th width="20%">&nbsp;</th>
					  	<td width="30%">&nbsp;</td>
					</tr>
