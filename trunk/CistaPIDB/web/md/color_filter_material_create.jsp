...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
		<td class="pageTitle">Master Data :: Create Color Filter Material</td>
  </tr>
</table>
			<div class="content">
			<form id="createForm" action="<%=cp%>/md/color_filter_material_create.do" method="post">
					  ...
					  <td width="30%"><input class="text" readonly
					      ...
					      <img src="<%=cp%>/images/lov.gif" alt="LOV" id="projCodeWVersionSSBtn">
							...
							SmartSearch.setup({
								...
								columns:"PROJ_CODE,PROJ_CODE_W_VERSION",
								orderBy:"PROJ_CODE",
                                title:"Project Code w Version",
                                autoSearch:false,
                                mode:0,
								callbackHandle:"getProjectCodeCallback"
							});

							function getProjectCodeCallback(inputField, columns, value) {
								if ($(inputField) && value != null && value.length > 0) {
									...
									for(var i = 0; i < value.length; i++) {
										temp2Value += "," + value[i][columns[0]];
									}
							
									if(temp2Value != "") {
										$("projCode").value = temp2Value.substring(1);
									}
								}
							}
							</script>
