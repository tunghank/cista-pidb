...
<%@ page import="com.cista.pidb.md.to.CspQueryTo" %>
...
<script type="text/javascript">
	...
	function createWithRef() {
		if (!$('ref') || getRadioValue('ref') == null) {
			...
			document.location.href='<%=cp%>/md/csp_create.do?m=pre&ref=' + getRadioValue('ref');
		}
	}
</script>
</head>
<body>
<% 	List<CspTo> result = (List<CspTo>)request.getAttribute("result");
	CspQueryTo criteria = (CspQueryTo)request.getAttribute("criteria");
%>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="pageTitle">Master Data :: CSP Package Query Result</td>
  </tr>
</table>
			<div class="content">
				...
				<tr>
					...
					<div align="right">
					  ...
					  <input name="button1" type="button" class="button" id="button1"value="Create New" 
					 	 onClick="document.location.href='<%=cp%>/md/csp_create.do?m=pre'">
					  ...
					  onClick="document.location.href='<%=cp%>/md/csp_query.jsp'">
					</div>
					</td>
				</tr>
			</table>
	...
	<form name="pagingForm" action="<%=cp %>/md/csp_query.do" method="post">
...
%>
							<tr>
								...
								<td <%=tdcss %>>
								<a href="<%=cp%>/md/csp_edit.do?m=pre&pkgName=<%=BeanHelper.getHtmlValueByColumn(oneIc, "PKG_NAME") %>"><%=BeanHelper.getHtmlValueByColumn(oneIc, "PROJ_NAME")%></a>&nbsp;</td>
