 ...
 * any wizard:
 *  - tabbed display - keyboard navigation through tabs - tab navigation bar (<button>-s) -
 * standard/advanced mode (ability to hide some elements in standard mode) -
 * data validation code
 * 
 * Call the constructor like this:
 * 
 * \code var args = { tabsID : "tabs", tabBarID : "tab-bar" }; var wizard = new
 * Zapatec.Wizard(args); \endcode
 * 
 ...
 * tabBarID -- must be the ID of an element where the tab bar should be
 * inserted.
 * 
 * @param args
 *            [object] contains the arguments to be used, as described above.
...
/**
 * Initializes the wizard. Developers should call this function after creating a
 * wizard and assigning any event handlers and/or creating the standard toolbar
 * (with Zapatec.Wizard.setupNav()).
 */
...
Zapatec.Wizard.prototype.changeTab = function(newTab) {
	...
	if (currentTab != newTab) {
		if (!currentTab || this.onBeforeTabChange(currentTab, newTab)) {
			...
			if (this.btnHome)
				this.btnHome.style.visibility = this.isFirstTab()
						? "hidden"
						: "visible";
			if (this.btnPrev)
				this.btnPrev.style.visibility = this.isFirstTab()
						? "hidden"
						: "visible";
			if (this.btnNext)
				this.btnNext.style.visibility = this.isLastTab()
						? "hidden"
						: "visible";
			if (this.btnEnd)
				this.btnEnd.style.visibility = this.isLastTab()
						? "hidden"
						: "visible";
			if (this.btnAdvanced)
 ...
 * involves displaying or hiding any fields that have the class "advanced". This
 * is actually achieved by simply adding or removing the class to those
 * elements, accordingly.
 */
Zapatec.Wizard.prototype.toggleAdvanced = function() {
	var tab = this.tabs[this.getCurrentTab()], a = tab.advanced_els, el, i = 0, visible = (tab.advanced = !tab.advanced);
	while (el = a[i++]) {
...
/**
 * Creates a default navigation bar for the wizard and appends it into the given
 * parent. The default buttons are:
 *  - "advanced mode" (only visible if the current tab has advanced elements) -
 * "Begin" (moves to first tab) - "Prev." (moves to previous tab) - "Next"
 * (moves to next tab) - "End" (moves to last tab)
 * 
 * Any of these buttons is assigned a certain class name, which helps
 * customizing the look through external CSS. The classes are (in the same order
 * as above): "btn-advanced", "btn-begin", "btn-prev", "btn-next", "btn-finish".
 * 
 * @param parent
 *            [HTMLElement] the parent of the tab navigation bar.
 * 
 * @return [HTMLElement] a reference to the DIV containing the navigation bar.
 */
Zapatec.Wizard.prototype.setupNav = function(parent) {
	var div = Zapatec.Utils.createElement("div", parent
					|| this._tabsEl.parentNode);
...
/**
 * Create a custom validator. You need to specify the ID of the validator, and a
 * function that does the validation checks. The function specification is
 * simple:
 * 
 * \code function validate(value, args, validator); \endcode
 * 
 * The 3 arguments are:
 *  - value -- the value that we should check validation against; usually the
 * \em value attribute of the input field. - args -- any arguments that might be
 * passed to the validator in the class name. Note that this might be null. -
 ...
 *  - the list of advanced elements (those that should be visible only if the
 * tab is in "show advanced options" mode). - the list of fields that require
 * validation. Also, all these fields get installed an "onblur" handler that
 * checks validation.
 * 
 * @param tab
 *            reference to the internally defined tab object.
 */
	...
	while (el = a[i++]) {
		...
		if (/(^|\s)validate-([^\s-]+)(-[^\s]+)?/i.test(c)) {
			...
			el.onblur = function(ev) {
				ev || (ev = window.event);
				return self._validateField(this, this.__msh_validator,
 ...
 * 
 * @param field
 *            [HTMLElement] a reference to a HTMLInputElement (usually) to be
 *            validated.
 * @param validator
 *            [string] the ID of a validator.
 * @param args
 *            [string] the validator arguments, as specified in the class name.
 * @param event
 *            [Event] the Event object, useful if we want to stop propagation.
 */
Zapatec.Wizard.prototype._validateField = function(field, validator, args,
		event) {
		...
		if (tag == "input" || tag == "select" || tag == "textarea") {
			...
			if (typeof message == "string") {
				...
				if (!div) {
					...
					div = field.__msh_message = Zapatec.Utils
							.createElement("div");
 ...
 * 
 * @param tab
 *            [optional, Object] a reference to the tab to check the state
...
Zapatec.Wizard.prototype._updateAdvancedButton = function(tab) {
	if (this.btnAdvanced) {
		...
		this.btnAdvanced.innerHTML = tab.advanced
				? "Hide advanced options"
				: "Show advanced options";
		this.btnAdvanced.style.visibility = tab.advanced_els.length == 0
				? "hidden"
				: "visible";
	}
};
 ...
 * following:
 * 
 * \code function myOnInit() { this.changeTab("tab-3"); alert("Look, we changed
 * to the third tab! >8-]"); }; wizard.onInit = myOnInit; \endcode
 */
// @{
// / A "do nothing" handler used for default event hooks.
Zapatec.Wizard._doNothing = function() {
	return true;
};
