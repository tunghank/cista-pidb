...
/**
 * Moves elelement to the specified position. To be moved element should be
 * positioned absolute and should be child of 'within' element.
 * 
 * @param el
 *            [HTMLElement] - the element to move
 * @param x
 *            [number] - the x coordinate to move to
 * @param y
 *            [number] - the y coordinate to move to
 * @param within
 *            [HTML element] - element to move within (default is document.body)
 * @return true if successful, otherwise false
 */
Zapatec.Utils.moveElementTo = function(el, x, y, within) {
	if (!el.within || el.within != within)
		return false;
...
/**
 * Prepares elelement to be moved by Zapatec.Utils.moveElementTo. To be moved
 * element should be positioned absolute and should be direct child of the BODY,
 * should have margin set to 0px.
 * 
 * @param el
 *            [HTMLElement] - the element to move
 * @param restorer
 *            [SRProp object] - the SRProp object you want to use to restore our
 *            elements properties, if you have already one.
 * @param within
 *            [HTML element] - if you want to move the element within some other
 *            element, pass the refference to it through this var (default is
 *            document.body)
 * @return true if successful, otherwise false
 */
Zapatec.Utils.prepareToMove = function(el, restorer, within) {
	...
	// no other elments in the parent elements tree can not have any
	// positioning,
...
/**
 * Restores all the changes made in thr elelement by
 * Zapatec.Utils.preapareToMove. absolute and should be direct child of the
 * BODY, should have margin set to 0px.
 * 
 * @param el
 *            [HTMLElement] - the element to move
 * @return true if successful, otherwise false
 ...
 * fully depends on three above functions and currently will be used for another
 * method below. To work this function has requirements to the element and its
 * 'within' element: element should be positioned absolutely, have margin 0px;
 * 'within' element should be positioned relatively;
 * 
 * @param el
 *            [HTML element] - we are getting the position of this element
 * @param within
 *            [HTML element] - within this one (default is document.body)
...
/**
 * Moves element by the given offset in the 'within' element. Function is
 * closely connected to above functions.
 * 
 * @param el
 *            [HTML element] - element to move
 * @param offsetX
 *            [number] - x offset to move for
 * @param offsetY
 *            [number] - y offset to move for
 * @param within
 *            [HTML element] - element to move within
 * @return true if succeeds, otherwise false
 */
Zapatec.Utils.moveElementFor = function(el, offsetX, offsetY, within) {
	var oldPos = Zapatec.Utils.getElementPos(el, within);
	if (oldPos) {
		return Zapatec.Utils.moveElementTo(el, oldPos.x + offsetX, oldPos.y
						+ offsetY, within);
	} else {
		return false;
	}
...
/**
 * returns element Array which has attribute 'attr' with value 'val' by giving
 * 'el' you can finetune your search
 * 
 * @param attr
 *            [string] attribute to search
 * @param val
 *            [string OR number] searched attributes value, ignored if 0.
 * @param el
 *            [HTMLElement] reference to the element.
 * @param recursive
 *            [boolean] searches in childs.
 */
Zapatec.Utils.getElementsByAttribute = function(attr, val, el, recursive) {
	...
	while (a) {
		...
		if (recursive && a.hasChildNodes()) {
			retArray = retArray.concat(Zapatec.Utils.getElementsByAttribute(
					attr, val, a, recursive));
		}
		a = a.nextSibling
	};
	return retArray
...
/**
 * verifies is the given variable object or object id and returns the object or
 * false if either
 * 
 * @param el
 *            [HTMLElement or string] reference to the element or element id
 * @param errorMessage
 *            [string] message shown in statusbar if el is not correct object
 * @param errorAction
 *            [function reference] called function if el is not correct object
...
Zapatec.Utils.idOrObject = function(el, errorMessage, errorAction) {
	...
	// if still this isn't an HTMLElement firing the error, othewise returning
	// it
 ...
 * this is not good for images with changed size, because you cant change the
 * background-image size and it will tile.
 * 
 * @param el
 *            [HTMLElement] reference to the element
 * @return the needed element
...
Zapatec.Utils.img2div = function(el) {
	// FIX ME: maybe this can be changed to div/img from just div with
	// background
...
/**
 * object for saving and restoring properties of the given object - SRProp (Save
 * Restore Object Properties)
 * 
 * @param obj
 *            [object] - object to work with this class adds methods to any
 *            given object which enable to save and restore properties using
 *            following \code var object = {}; ob = new
 *            Zapatec.Utils.SRProp(obj); object.prop = ""; ob.saveProp("prop");
 *            object.prop = "ttt"; ob.restoreProp("prop"); ob.restoreAll();
 *            \endcode
 */
 ...
 * 
 * @param propName
 *            [string] - name of the property to save - can be the followin
 *            "prop", "level1.level2" and so on.
 * @return true if successful, otherwise false
 */
 ...
 * 
 * @param propName
 *            [string] - name of the property to restore - can be the followin
 *            "prop", "level1.level2" and so on.
 * @return true if successful, otherwise false
 */
Zapatec.Utils.SRProp.prototype.restoreProp = function(propName) {
	// property name should definately be string, I think :)
	if (typeof propName != "string"
			|| typeof this.savedProp[propName] == "undefined") {
		return false;
	}
	var self = this;
	try {
		// For HTMLElements it is impossible to just set such property as
		// parentNode and nextSibling
		// so we first try to do appendChild of the parentNode element
		// if it isn't HTMLElement we just restreing it using the old good eval
		// again :)
		if (propName.indexOf("parentNode") != -1 && this.savedProp[propName]
				&& typeof this.savedProp[propName] == "object"
				&& this.savedProp[propName].appendChild) {
			this.savedProp[propName].insertBefore(self.obj,
					this.savedProp[propName
							.replace(/parentNode/, "nextSibling")]);
		} else {
 ...
 * 
 * @param propName
 *            [string] - name of the property to get - can be the followin
 *            "prop", "level1.level2" and so on.
 */
 ...
 *            [HTMLElement] reference to the element.
 * @param objArgs
 *            [object] must be object with the following properties: left
 *            [number] draggable area left edge in pixels according to the
 *            dragLayer or document.body. top [number] draggable area top edge
 *            -''-. right [number] draggable area right edge -''-. bottom
 *            [number] draggable area bottom edge -''-. direction [String
 *            'horizontal'/'vertical'] enables dragging only described direction
 *            followShape [boolean] draggable area controls the object size (not
 *            only the top left position). handler [HTMLElement] reference to
 *            the handler element (fe. window titlebar). dragCSS [string]
 *            className for dragstate (will be removed after release)
 *            overwriteCSS [string] this will overwrite the current elements
 *            className and will be restored only after calling restorePos()
 *            dragLayer [HTMLElement] reference to the element in which we are
 *            dragging the draggable element, default is el.parentNode. method
 *            [string] cut, copy, dummy, slide dropname [string] defines name of
 *            the droparea stopEv [boolean] determines if our event handlers
 *            should stop the event propogation onDragInit [function] the
 *            handler for dragStart called in the end of the event onDragMove
 *            [function] the handler for dragMove called in the end of the event
 *            onDragEnd [function] the handler for dragEnd called in the end of
 *            the event beforeDragInit [function] the handler for dragStart
 *            called at the beginning of the event beforeDragMove [function] the
 *            handler for dragMove called at the beginning of the event
 *            beforeDragEnd [function] the handler for dragEnd called at the
 *            beginning of the event eventCapture [boolean] I don't think this
 *            should be documented as currently this is used as a hack to cacth
 *            events before DnD, I used it in Window script. So basicaly this is
 *            for programmers :)
 */
Zapatec.Utils.Draggable = function(el, objArgs) {
	...
	// Init works only with its objArgs object so let's not bother it with new
	// arguments :)
...
Zapatec.Utils.Draggable.prototype.init = function(objArgs) {
	...
	// through this object will be done all the configuration and behaviour of
	// the element
	...
	if (el.dragAtr.handler) {
		el.dragAtr.handler = Zapatec.Utils.idOrObject(el.dragAtr.handler,
				'cannot find the handlerobject:' + el.dragAtr.handler);
	}
	if (el.dragAtr.handler) {
		el.dragAtr.handler = Zapatec.Utils.img2div(el.dragAtr.handler);
	};
	// getting dragLayer
	el.dragAtr.dragLayer = Zapatec.Utils.idOrObject(el.dragAtr.dragLayer,
			'cannot find the dragLayer:' + el.dragAtr.dragLayer)
			|| document.body;
	...
	// The rest of event are tied to the div which covers the whole screen and
	// has big z-index.
	// This way we can achieve many advantages, like giving the right cursor,
	// reactiong only on our events, solving problem with iframes on the page,
	// etc.
	var self = this;
	if (Zapatec.is_gecko) {
	...
	// if the initialization was successfull need to save and restore some of
	// the elements properties
	// What a nice surprice - we have a class for it!!! ;)
	this.restorer = new Zapatec.Utils.SRProp(this.draggable);
	// need to save some properties of the element which we need to restore (if
	// we'll need)
...
/**
 * searches elements with className=class and makes them draggable(useful to
 * call it on document load) @ param class [string] searcable element's
 * CSSclassname. @ param el [HTMLElement] reference to the element. @ param
 *               recursive [boolean] searches in childs.
 */
Zapatec.Utils.initDragObjects = function(className, el, recursive, attribObject) {
	if (!className)
		return;
	el = Zapatec.Utils.idOrObject(el);
	var changeArray = Zapatec.Utils.getElementsByAttribute('className',
			className, el, recursive);
...
Zapatec.Utils.Draggable.prototype.dragStart = function(ev) {
	...
	this.dragging = true;
	// to work with the draggable element is easier using just 'el' and not
	// 'this.draggable' :)
	...
	// mousemove and mouseup event handlers to block all the page behind while
	// we are dragging
	if (!el.dragAtr.cover) {
		// creating a div
		...
		// This is the workaround for IE who doesn't firest needed events if the
		// element is empty
...
Zapatec.Utils.Draggable.prototype.dragMove = function(ev) {
	...
	if (el.dragAtr.direction != "vertical") {
		...
		if (el.dragAtr.left || el.dragAtr.left === 0) {
			diffX = el.dragAtr.left
					- (newX + Zapatec.Utils.getElementPos(el,
							el.dragAtr.dragLayer).x);
		} else {
			diffX = 0;
		}
		if (diffX > 0) {
			newX = el.dragAtr.left
					- Zapatec.Utils.getElementPos(el, el.dragAtr.dragLayer).x;
		} else {
			if (el.dragAtr.right || el.dragAtr.right === 0) {
				diffX = el.dragAtr.right
						- (el.dragAtr.followShape ? el.offsetWidth : 0)
						- (newX + Zapatec.Utils.getElementPos(el,
								el.dragAtr.dragLayer).x);
			} else {
				diffX = 0;
	...
	if (el.dragAtr.direction != "horizontal") {
		...
		if (el.dragAtr.top || el.dragAtr.top === 0) {
			diffY = el.dragAtr.top
					- (newY + Zapatec.Utils.getElementPos(el,
							el.dragAtr.dragLayer).y);
		} else {
			diffX = 0;
		}
		if (diffY > 0) {
			newY = el.dragAtr.top
					- Zapatec.Utils.getElementPos(el, el.dragAtr.dragLayer).y;
		} else {
			if (el.dragAtr.bottom || el.dragAtr.bottom === 0) {
				diffY = el.dragAtr.bottom
						- (el.dragAtr.followShape ? el.offsetHeight : 0)
						- (newY + Zapatec.Utils.getElementPos(el,
								el.dragAtr.dragLayer).y);
			} else {
				diffX = 0;
...
/**
 * returns element/its copy what will be drags It depends on method property of
 * draggable object Cut method - this is how the DnD currently works. You drag
 * the element itself. By 'element' I mean the HTML element which is passed to
 * the constructor. Copy method - by this method element is not touched at all,
 * DnD creates its copy and drags it. When you stopped dragging the copy stays
 * at the place it was dragged to and can not be moved anymore (this is for now,
 * we'll change this later). Dummy method - the DnD should behave similar to the
 * copy method, but with one little difference - it will destroy the copy after
 * the dragging ended.
 */
 ...
 * @param dropname
 *            [string] id for simple dnd
 * @param ondrop
 *            [function] fires on drop
 * @param ondraginit
 *            [function] fires on any dragging start
 * @param ondragover
 *            [function] fires on drag over the droparea
 * @param odragout
 *            [function] fires on drag out of the droparea
 * @param ondragend
 *            [function] fires on any dragging end
 */
Zapatec.Utils.DropArea = function(el, dropname, ondrop, ondraginit, ondragover,
		ondragout, ondragend) {
...
Zapatec.Utils.DropArea.prototype.dragInit = function(el) {
	...
	// retreiving the mouse position, we don't have event object, but I think we
	// don't need it :)
	var mousePos = {
		x : el.dragAtr.mouseX,
		y : el.dragAtr.mouseY
	...
	if ((mousePos.x > dropPos.x)
			&& (mousePos.x < dropPos.x + this.dropArea.offsetWidth)
			&& (mousePos.y > dropPos.y)
			&& (mousePos.y < dropPos.y + this.dropArea.offsetHeight)) {
		this.elementOver = el;
	}
	// if we have dragInit handler fire it
	this.fireEvent("onDragInit", el, this);
	// if we have dragOver handler and eventually the element was over the
	// droparea on the start
	// lets fire it
	if (this.elementOver) {
...
Zapatec.Utils.DropArea.prototype.removeListeners = function() {
	...
	Zapatec.Utils.Draggable.globalEvents
			.removeEventListener(this.tmp.onDragInit);
	Zapatec.Utils.Draggable.globalEvents
			.removeEventListener(this.tmp.onDragMove);
	Zapatec.Utils.Draggable.globalEvents
			.removeEventListener(this.tmp.onDragEnd);
}
...
Zapatec.Utils.DropArea.prototype.addListeners = function() {
	...
	Zapatec.Utils.Draggable.globalEvents.addEventListener("onDragInit",
			this.tmp.onDragInit);
	Zapatec.Utils.Draggable.globalEvents.addEventListener("onDragMove",
			this.tmp.onDragMove);
	Zapatec.Utils.Draggable.globalEvents.addEventListener("onDragEnd",
			this.tmp.onDragEnd);
}
