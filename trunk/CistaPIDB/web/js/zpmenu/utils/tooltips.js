 ...
 * tooltip element which can be anywhere in the DOM tree to some target element.
 * Then, when the end-user hovers the target element, the tooltip will appear
 * near the mouse position.
 * 
 * @param target
 *            [HTMLElement or string] reference to or ID of the target element
 * @param tooltip
 *            [HTMLElement or string] reference to or ID of the tooltip element
 * 
 ...
 * 
 * \code <dfn title="Tooltip title"> Tooltip contents </dfn> \endcode
 * 
 ...
 * 
 * The optional "class_re" parameter allows one to filter elements by some class
 * name. It can be a RegExp object or a string; if it's a string, only DFN-s
 * containing that string in the value of the "class" attribute will be
 * considered. If it's a RegExp, only those DFN-s where the value of the class
 * attribute matches the RegExp will be considered.
 * 
 * The DFN tag is a standard HTML element. It's purpose is to markup a \em
 * definition, which seems fairly close to the purpose of a tooltip.
 * 
 * @param class_re
 *            [string or RegExp, optional] -- filter the DFN elements that
 *            display tooltip
 */
...
Zapatec.Tooltip.prefs = {
	move : false
/** < If this is true then tooltips will move with mouse */
};
Zapatec.Tooltip._C = null;
/** < [internal] keeps a reference to the currently displayed tooltip */

/**
 * Called automatically when "onmouseover" or "onmousemove" occurs on the target
 * element. This function takes care to display the tooltip, if not already
 * visible, and to clear the timeout if the tooltip was set to hide.
 * 
 * @param ev
...
Zapatec.Tooltip.prototype._onMouseMove = function(ev) {
	...
	if (!this.visible && !Zapatec.Utils.isRelated(this.target, ev)) {
		var x = ev.clientX + 2, y = ev.clientY + 4;
		this.show(x, y);
	}
};
 ...
 * function sets a timeout that will hide the tooltip in 150 milliseconds. This
 * timeout can be cancelled if during this time the mouse returns to the target
 * element or enters the tooltip element.
 * 
 * @param ev
 *            [Event] the event object.
 */
