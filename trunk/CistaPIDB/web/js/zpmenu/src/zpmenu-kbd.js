...
Zapatec.Menu.onDocumentKeyDown = function(ev) {
	...
	var buttonEsc = function(item) {
		...
		if (item.__zp_subtree && item.__zp_state) {
			...
			if (subtree.__zp_activeitem == item.__zp_item)
			...
			if (item.__zp_subtree) {
				...
				if (subMenu.__zp_items.length > 0) {
					if (subMenu.__zp_activeitem) {
						Zapatec.Menu
								.unselectItem(self.items[subMenu.__zp_activeitem]);
					}
					subMenu.__zp_activeitem = subMenu.__zp_items[0];
					Zapatec.Menu
							.selectItem(self.items[subMenu.__zp_activeitem]);
				}
			} else if (!self.config.vertical
					&& !/zpMenu-level-1/.test(item.className)) {
				// Go to next top menu item
				item.onmouseout();
				return buttonRightArrow(self.items[self
						._getTree(item.__zp_tree).__zp_activeitem]);
			} else {
				Zapatec.Menu.selectedItemsStack.push(item);
			}
		}
	...
	var buttonUpArrow = function(item) {
		...
		if (!self.config.vertical && /zpMenu-level-1/.test(item.className)) {
			...
			var prevItem = item.previousSibling;
			while (prevItem
					&& (!/zpMenu-item/i.test(prevItem.className) || /zpMenu-item-hr/i
							.test(prevItem.className))) {
	...
	var buttonDownArrow = function(item) {
		...
		if (!self.config.vertical && /zpMenu-level-1/.test(item.className)) {
			...
			var nextItem = item.nextSibling;
			while (nextItem
					&& (!/zpMenu-item/i.test(nextItem.className) || /zpMenu-item-hr/i
							.test(nextItem.className))) {
		...
		if (subtree && subtree.__zp_keymap) {
			...
			if (keymapItem) {
				...
				if (item && !item.__zp_state) {
					Zapatec.Utils.removeClass(item, "zpMenu-item-selected");
					Zapatec.Utils.removeClass(item,
							"zpMenu-item-selected-collapsed");
					if (subtree.__zp_activeitem == item.__zp_item)
						subtree.__zp_activeitem = '';
				}
...
Zapatec.Utils.addEvent(window.document, 'keydown',
		Zapatec.Menu.onDocumentKeyDown);

// Disable scrolling with arrows
Zapatec.Utils.addEvent(window.document, 'keypress', function(ev) {
			ev || (ev = window.event);
			switch (ev.keyCode) {
				case 37 : // Left arrow
				case 39 : // Right arrow
