/* grid to Excel JavaScript by ExtJs, version 1.0.0.1
 * 
 */
...
function post_to_url(path, params, method)
{
    ...
    form.setAttribute("isUpload", "true");
...
//      Generate the data rows from the data in the Store
        for (var i = 0, it = this.store.data.items, l = it.length; i < l; i++) {
            ...
            for (var j = 0; j < cm.getColumnCount(); j++) 
            {
                ...
                {
                    var vOrg = r[cm.getDataIndex(j)]; //grid 資料
                    //add by jere 若有特殊html, 取代為\r
                    var v = "";
                    if(typeof(vOrg)=='string')
                    {
                    	v = vOrg.replace(/<br>/g, "\r");
                    }
                    else
                    {
                    	v = vOrg;
