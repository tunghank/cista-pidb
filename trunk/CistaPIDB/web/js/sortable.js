/**
	Usage:
		add onclick event on table header(td)
		parameters:
		this: always write this.
		ih: ignore header lines.
		columnIdx: column index which to sort.(optional)
	PS: Please keep table in following hierarchy:
	<table>
		<tr><td onclick="sort(this, 1)"></td>...</tr>
		....
		<tr><td></td>...</tr>
	</table>
*/
...
function quicksortIncrease(s, c, l, r) {
	...
	while(true) {
		do {
			i ++;
		} while(i<=r && comp(s[i].cells[c].innerText, s[pivot].cells[c].innerText)>0)

		do {
			j --;
		} while(j>=l && comp(s[j].cells[c].innerText, s[pivot].cells[c].innerText)<0)
...
function compareDateTime(dt1, dt2) {
	var result1=dt1.match(/^(\d{1,4})(.|-|\/)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})$/);
	var dt1= new Date(result1[1], result1[3]-1, result1[4], result1[5], result1[6], result1[7]);
	var result2=dt2.match(/^(\d{1,4})(.|-|\/)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})$/);
	var dt2= new Date(result2[1], result2[3]-1, result2[4], result2[5], result2[6], result2[7]);
...
{
	var result=t.match(/^(\d{1,4})(.|-|\/)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})$/);
	if(result==null) return false;
