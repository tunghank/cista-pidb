...
<%
	List<FtTestProgramTo> toList = (List)request.getAttribute("selectList");
%>
<html>
<head>
<!-- This is header -->
...
<table border=0 cellpadding=0 cellspacing=0 width="100%"><tr><td>
			<table class="grid" border="0" cellpadding="1" cellspacing="1">
				...
				<%
					int idx = 0;
					for(FtTestProgramTo to : toList) {
						idx ++;
						String tdcss = "class=\"c" + idx % 2+"\"";
						String chk = to.getFtTestProgName();
						%>
				<tr>
					<td <%=tdcss %>><input type="checkbox" value="<%=chk %>" name=chkbox></td>
					<td <%=tdcss %>><%=to.getPartNum() %></td>
					<td <%=tdcss %>><%=to.getFtTestProgName() %></td>
				</tr>
						<%
					}
				%>
