...
<%
	List<SapMasterIcTypeTo> smcList = (List)request.getAttribute("selectList");
%>
<html>
<head>
<!-- This is header -->
...
<body>
<form name="selectSmc" action="<%=cp %>/dialog/select_sapMasterIcType.do" method="post">
...
<div id="resultPanel" style="overflow:auto;width:100px;height:100px">
			<table class="grid" border="0" cellpadding="1" cellspacing="1">
				...
				<%
					int idx = 0;
					for(SapMasterIcTypeTo smc : smcList) {
						idx ++;
						String tdcss = "class=\"c" + idx % 2+"\"";
						String userStr = smc.getIcType()+"|"+smc.getDescription();
						%>
				<tr>
					<td <%=tdcss %>><input type="checkbox" value="<%=userStr %>" name=SmcCheckbox></td>
					<td <%=tdcss %>><%=smc.getIcType() %></td>
					<td <%=tdcss %>><%=smc.getDescription() %></td>
				</tr>
						<%
					}
				%>
