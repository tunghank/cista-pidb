...
<%
	List<SapMasterVendorTo> smcList = (List)request.getAttribute("selectList");
...
<body>
<form name="selectSmc" action="<%=cp %>/dialog/select_sapMasterVendorByName.do?m=list" method="post">
	...
	<tbody>
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="pageTitle">Dialog :: Select Vendor</td>
				</tr>
			</table>
			<div class="content">
			<table class="formErrorAndButton">
				<tr>
					<td>
					<div class="formErrorMsg" id="error"><html:errors/><!--ErrorMessage--><%=smcList==null || smcList.size()==0?"No Vendor.":"" %>&nbsp;</div>
					</td>
				</tr>
				<tr>
					<td>
...
<div id="resultPanel" style="overflow:auto;width:100px;height:100px">
			<table class="grid" border="0" cellpadding="1" cellspacing="1">
				...
				<%
					int idx = 0;
					for(SapMasterVendorTo smc : smcList) {
						idx ++;
						...
						%>
				<tr>
					<td <%=tdcss %>><input type="checkbox" value="<%=smc.getVendorCode() %>|<%=smc.getShortName() %>" name=SmcCheckbox></td>
					<td <%=tdcss %>><%=(smc.getVendorCode()==null)? "" : smc.getVendorCode() %></td>
					<td <%=tdcss %>><%=(smc.getShortName()==null)? "" : smc.getShortName() %></td>
				</tr>
						<%
					}
				%>
