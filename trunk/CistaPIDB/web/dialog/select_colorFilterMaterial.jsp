...
<%
	List<ColorFilterMaterialTo> smcList = (List)request.getAttribute("colorFilterList");
...
function getMaxCFMaterialNum(){
	new Ajax.Request( 
	'<%=cp%>/ajax/fetch_maxVariantColorFilter.do?materialNum='+$('materialNum').value,
	{
		method: 'post',
	   //FCG1
		//parameters: 'projCodeWVersion=' + $('projCodeWVersion').value+'&remark='+$F('remark')+'&colorFilterDesc='+$F('colorFilterDesc'),
		parameters: 'projCodeWVersion=' + $('projCodeWVersion').value+'&remark='+$F('remark')+'&colorFilterDesc='+$F('colorFilterDesc')+'&mpStatus='+$F('mpStatus'),
...
<form id="cFMaterialCreateForm" name="selectEOLCust" action="<%=cp %>/md/cp_material_edit.do?m=save" method="post">
	...
	<tbody>
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="pageTitle">Dialog :: Color Filter Material Modify</td>
				</tr>
			</table>
			<div class="content">
			<table class="formErrorAndButton">
				<tr>
					<td>
					<div class="formErrorMsg" id="error"><html:errors/><!--ErrorMessage--><%=smcList==null || smcList.size()==0?"No Color Filter Material Data.":"" %>&nbsp;</div>
					</td>
				</tr>
			</table>
			...
			<div id="resultPanel" style="overflow:auto;width:100px;">
						<table class="grid" border="0" cellpadding="1" cellspacing="1">
							...
							<tr>
								...
								<th>Color Filter Material Desc:
								  <input type="text" class="text" name="colorFilterDesc" id="colorFilterDesc" value="">
								</th>
								<th>
									<!-- //FCG1 -->
									<%if(smcList != null && smcList.size() > 0){ %>
									<input name="addBtn" type="button" class="add_button" id="addBtn" value="Add" disabled>
									<%}else{ %>
						...
						<tr><td>
									...
									<%if (smcList != null && smcList.size() > 0) {%>
										<%
											...
											for(ColorFilterMaterialTo material : smcList) {
												if (material != null){
											idx ++;
											String tdcss = "class=\"c" + idx % 2+"\"";
										%>
										<tr>
										<td <%=tdcss %>><%=material.getColorFilterMaterialNum()%>&nbsp;
											<input name='colorFilterMaterialNum' type='text' style="display: none;" value='<%=BeanHelper.getHtmlValueByColumn(material, "COLOR_FILTER_MATERIAL_NUM") %>'>
										</td>
...
function onAddDesc(){
	...
	if ($('colorFilterDesc').value==""){
		alert("DESCRIPTION must field");
		return;
	}
	
	getMaxCFMaterialNum();
	$('colorFilterDesc').value="";
}
