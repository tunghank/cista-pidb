...
<%
	List<SapMasterPackageTypeTo> smcList = (List)request.getAttribute("selectList");
%>
<html>
<head>
<!-- This is header -->
...
<body>
<form name="selectSmc" action="<%=cp %>/dialog/select_sapMasterPackageType.do" method="post">
...
<table border=0 cellpadding=0 cellspacing=0 width="100%"><tr><td>
			<table class="grid" border="0" cellpadding="1" cellspacing="1">
				...
				<%
					int idx = 0;
					for(SapMasterPackageTypeTo smc : smcList) {
						idx ++;
						String tdcss = "class=\"c" + idx % 2+"\"";
						if (smc == null) {
							break;
						}
						String userStr = smc.getPackageType()+"|"+smc.getDescription();
						%>
				<tr>
					<td <%=tdcss %>><input type="checkbox" value="<%=userStr %>" name=SmcCheckbox></td>
					<td <%=tdcss %>><%=(smc.getPackageType()==null)? "" : smc.getPackageType() %></td>
