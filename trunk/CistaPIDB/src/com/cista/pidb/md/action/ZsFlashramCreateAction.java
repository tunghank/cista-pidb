...
public class ZsFlashramCreateAction extends DispatchAction {
	...
	public ActionForward pre(final ActionMapping mapping,
		...
		if (request.getParameter("ref") != null) {
			...
			ZsFlashramTo flashramTo = new ZsFlashramDao()
					.findByPrimaryKey(materialNum);
			request.setAttribute("ref", flashramTo);
		}
	...
	public ActionForward save(final ActionMapping mapping,
			...
			final HttpServletResponse response) {
		String forward = "save";
		ZsFlashramDao falshramDao = new ZsFlashramDao();
		...
		flashramTo.setMaterialNum(materialNum);
		...
		flashramTo.setCreatedBy(loginUser.getUserId());

		falshramDao.insertFlashram(flashramTo);

		/***********************************************************************
		 * Release to ERP Add 2008/06/16 Hank Added
		 **********************************************************************/
		final UserTo user = loginUser;
		final ZsFlashramTo flashram = flashramTo;
		...
		if ("1".equals(toErp.trim())) {
			...
			TransactionCallbackWithoutResult callback = new TransactionCallbackWithoutResult() {
				public void doInTransactionWithoutResult(
					...
					String result = null;
					result = ZsReleaseERP.releaseFLASHRAM(flashram, user);
					if (result != null) {
