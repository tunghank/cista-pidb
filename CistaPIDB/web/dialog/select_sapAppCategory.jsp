...
<%
	List<SapAppCategoryTo> smcList = (List)request.getAttribute("selectList");
%>
<html>
<head>
<!-- This is header -->
...
<body>
<form name="selectSmc" action="<%=cp %>/dialog/select_sapAppCategory.do" method="post">
	...
	<tbody>
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="pageTitle">Dialog :: Select Sap App Category</td>
				</tr>
			</table>
			<div class="content">
			<table class="formErrorAndButton">
				<tr>
					<td>
					<div class="formErrorMsg" id="error"><html:errors/><!--ErrorMessage--><%=smcList==null || smcList.size()==0?"No Sap App Category.":"" %>&nbsp;</div>
					</td>
				</tr>
			</table>
<%
...
<div id="resultPanel" style="overflow:auto;width:100px;height:100px">
			<table class="grid" border="0" cellpadding="1" cellspacing="1">
				...
				<%
					int idx = 0;
					for(SapAppCategoryTo smc : smcList) {
						idx ++;
						String tdcss = "class=\"c" + idx % 2+"\"";
						if (smc == null) {
							break;
						}
						String userStr = smc.getApplicationCategory()+"|"+smc.getDescription();
						%>
				<tr>
					<td <%=tdcss %>><input type="checkbox" value="<%=userStr %>" name=SmcCheckbox></td>
					<td <%=tdcss %>><%=(smc.getApplicationCategory()==null)? "" : smc.getApplicationCategory()%></td>
