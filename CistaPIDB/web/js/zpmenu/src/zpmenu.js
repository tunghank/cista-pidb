...
Zapatec.Menu.prototype.onThemeLoad = function() {
	// Current trigger object that launched menu (menu can be attached to
	// several
	// objects).
 ...
 * @private
 * @param {object}
 *            objSource Source HTMLElement object
 */
Zapatec.Menu.prototype.loadDataHtml = function(objSource) {
	...
	this.id = objSource.id || Zapatec.Utils.generateID("tree");
	...
	if (this.config.vertical) {
		...
		// NOTE:zpMenu-vertical-mode and zpMenu-level-1 defines the top vertical
		// menu
	...
	objTheme.className = this.getClassName({
				prefix : 'zpMenu'
			});
 ...
 * Example:
 * // the following makes a tree for the
 * <ul id="tree-id">
 * element var tree = new Zapatec.Menu(&quot;tree-id&quot;);
 * // ... later
 ...
 * So in short, thisvariable remembers values returned by &quot;new
 ...
 *            list UL element
 * @param {object}
 *            parent Parent element that should hold the (sub)tree
 * @param {number}
 *            level Level of this (sub)tree in the main tree
 * @return ID of the (sub)tree
 * @type number
 */
Zapatec.Menu.prototype.createTree = function(list, parent, level) {
	var id;
	var intItem = 1, bFirst = true;

	if (list)
		id = list.id; // list can be null
	if (!id)
	...
	function _makeIt() {
		...
		next_li, // next <li>
		i = (list ? list.firstChild : null), items = parent.__zp_items = [];
		objMenu.trees[id] = parent;
		...
		while (i) {
			...
			if (i.nodeType != 1)
				...
				if (i.tagName.toLowerCase() == 'li') {
					last_li = objMenu.createItem(i, parent, next_li, level,
							intItem);
					if (last_li) { // false when webmaster creates malformed
									// tree
						// ONLY do odd/even for NON HR items
						// If HR items had odd/even then visual odd/even themes
						// (see zebra) look wrong
						if (!/zpMenu-item-hr/i.test(last_li.className)) {
							// this previously created item is NOT in the HR
							// class, create odd/even class
							strOddEven = "zpMenu-item-"
									+ (intItem % 2 == 1 ? "odd" : "even");
						...
						if (bFirst) {
							...
							bFirst = false;
							Zapatec.Utils
									.addClass(last_li, "zpMenu-item-first");
						}
		...
		if (last_li)
			Zapatec.Utils.addClass(last_li, "zpMenu-item-last");

		i = parent.firstChild;
		if (i && !level) {
			i.className = i.className.replace(/ zpMenu-lines-./g, "");
			i.className += (i === last_li)
					? " zpMenu-lines-s"
					: " zpMenu-lines-t";
		}
 ...
 *            li LI element
 * @param {object}
 *            parent Parent element where the HTML elements should be created
 * @param {object}
 *            next_li Next LI element, if this is not the last one
 * @param {number}
 *            level Level of this item in the main tree
 * @param {number}
 *            intItem Nth item for this sub-tree
 * @return DIV element holding the HTML elements of the created item
 * @type object
 */
Zapatec.Menu.prototype.createItem = function(li, parent, next_li, level,
		intItem) {
	if (!li.firstChild) {
		return;
	}
	var id = li.id || Zapatec.Utils.generateID("tree.item");
	var item = this.items[id] = Zapatec.Utils.createElement("div",
			parent.__zp_menu);
	...
	Zapatec.Utils.addClass(item, "zpMenu-level-" + (level + 1)); // Define
																	// the Nth
																	// level of
																	// a
																	// sub-menu,
	...
	Zapatec.Utils.addClass(item, "zpMenu-item-"
					+ (intItem % 2 == 1 ? "odd" : "even"));
	...
	var getAccessKey = function(node) {
		...
		if (node.nodeType == 1) { // ELEMENT_NODE
			if (key = node.getAttribute('accesskey')) {
				// Remove accesskey attribute because it will cause duplicate
				// onclick event
			...
			if (/_([a-z0-9])/i.test(label)) {
				label = label.replace(/_([a-z0-9])/i,
	...
	while (fc = li.firstChild) {
		if (fc.nodeType == 1 && (/^[ou]l$/i.test(fc.tagName.toLowerCase()))) {
			...
			if (!subtree) {
				...
				// The following to be able to position menu at the bottom right
				// corner
				...
				if (this.config.dropShadow) {
					var ds = np.__zp_dropshadow = Zapatec.Utils
							.createElement('div');
					...
					if (window.opera) {
						ds.style.backgroundColor = '#666'; // opacity doesn't
															// work in Opera
					} else {
						ds.style.filter = 'alpha(opacity='
			...
			if (fc.nodeType == 3) {
				...
				if (label) {
					...
					// IE 6.0 doesn't escape correctly plain text when it is
					// assigned to
					// innerHTML property
					...
					if (title)
						span.setAttribute('title', title); // To make title
															// work in Opera
				}
				...
				if (fc.tagName.toLowerCase() == 'img') {
					...
					} else if (fc.tagName.toLowerCase() == 'input'
							&& fc.getAttribute('type') == 'checkbox') {
					...
					td.appendChild(fc);
					if (title && !fc.getAttribute('title'))
						fc.setAttribute('title', title); // To make title
															// work in Opera
				}
			}
		}
 ...
 * @private
 * @param {object}
 *            item DIV element holding the item
 * @param {string}
 ...
 * @private
 * @param {object}
 *            item DIV holding the item elements
 * @param {object}
 *            img Optional. IMG element; normally one found in the
 *            <LI>
 */
...
/**
 * This function toggles an item if the state parameter is not specified. If
 * state is true then it expands the item, and if state is false then it
 * collapses the item.
 * 
 * @param {string}
 *            item_id Item ID
 * @param {boolean}
 *            state Optional. Desired item state
		...
		if (subtree) {
			if (state) {
				...
				for (var i = 0; i < subtree.__zp_items.length; i++) {
					...
					if (subtree.__zp_activeitem == subItemID)
						subtree.__zp_activeitem = '';
				}
			} else {
			...
			Zapatec.Utils.addClass(item, state
							? "zpMenu-item-expanded"
							: "zpMenu-item-collapsed");
		}
 ...
 * 
 * @param {string}
 *            item_id ID of the item to sync to
 */
 ...
 * @private
 * @param {string}
 *            tree_id ID of the subtree
 * @param {boolean}
 *            dont_call Pass true here if you don't want the subtree to be
 *            created
 ...
 * 
 * @param {object}
 *            triggerObject One of the following:
 * 
 * <pre>
 * element id [string] ||
...
Zapatec.Menu.prototype.setTriggerObject = function(triggerObject) {
						...
						if (typeof triggerElement == 'string') {
							...
							if (typeof triggerElement.triggerObject != 'undefined'
								...
								if (typeof triggerElement.triggerObject == 'string') {
									// Element id
									var objElement = document
											.getElementById(triggerElement.triggerObject);
									if (objElement) {
										objTriggerElements.push({
											triggerObject : objElement,
	...
	if (strTriggerEvent == 'mousedown' || strTriggerEvent == 'mouseup'
		...
		// Mouse trigger
		// Need this function to be able to set current trigger object and
		// arguments
		var funcSetupTriggerEvent = function(objTriggerElement) {
			Zapatec.Utils.addEvent(objTriggerElement.triggerObject, 'mouseup',
					function(objEvent) {
						...
						if (window.opera) {
							// Button 1 is used for both showing and hiding menu
							// in Opera
							// because Opera doesn't allow to disable context
							// menu
							if (button == 1
									&& objMenu.top_parent.style.display == 'none') {
								setTimeout(function() {
									// Set current trigger object
									objMenu.triggerObject = objTriggerElement.triggerObject;
									// Set arguments received from current
									// trigger object
									objMenu.triggerArgs = objTriggerElement.triggerArgs;
									// Show menu at mouse position
									objMenu.popupMenu(objMousePos.pageX,
											objMousePos.pageY);
								}, 100);
								return Zapatec.Utils.stopEvent(objEvent);
							}
						} else {
							// In Safari Meta (Alt) Key + left click is used
							// because it doesn't
							// react on right mouse button
							if (strTriggerKey == 'both'
									|| (strTriggerKey == 'left' && button == 1)
									|| ((!strTriggerKey || strTriggerKey == 'right') && (button > 1 || objEvent.metaKey))) {
								setTimeout(function() {
									// Set current trigger object
									objMenu.triggerObject = objTriggerElement.triggerObject;
									// Set arguments received from current
			...
			Zapatec.Utils.addEvent(objTriggerElement.triggerObject, 'keydown',
					function(objEvent) {
						...
						if (objEvent.keyCode == strTriggerKey) {
							// Set current trigger object
							objMenu.triggerObject = objTriggerElement.triggerObject;
							// Set arguments received from current trigger
							// object
							objMenu.triggerArgs = objTriggerElement.triggerArgs;
							// Show menu
							objMenu.popupMenu();
...
/**
 * Collection of animations (function references). These are called to
 * progressively style the DOM elements as menus show and hide. They do not have
 * to set item visibility, but may want to set DOM properties like clipping,
 * opacity and position to create custom effects.
 * 
 * @param {object}
 *            ref HTMLElement object that contains the menu items
 * @param {number}
 *            counter Animation progress value, from 0 (start) to 100 (end)
 */
...
Zapatec.Menu.animations.slide = function(ref, counter) {
	if (counter != 100) {
		...
		ref.style.marginTop = '-' + parseInt(ref.offsetHeight * (1 - cP))
				+ 'px';
		ref.style.clip = 'rect(' + parseInt(ref.offsetHeight * (1 - cP))
				+ 'px,' + ref.offsetWidth + 'px,' + ref.offsetHeight + 'px,0)';
	} else {
		if (typeof ref.__zp_origmargintop != 'undefined') {
...
Zapatec.Menu.animations.wipe = function(ref, counter) {
	if (counter != 100) {
		ref.style.clip = 'rect(0,'
				+ parseInt(ref.offsetWidth * (counter / 100)) + 'px,'
				+ parseInt(ref.offsetHeight * (counter / 100)) + 'px,0)';
	} else {
		try {
			ref.style.clip = '';
		} catch (objException) {
...
Zapatec.Menu.onItemMouseOver = function() {
	...
	while (item && item != document.body) {
		...
		if (t_id)
			tree = Zapatec.Menu.all[t_id];
		var itemClassName = item.className;
		if (/zpMenu-item/.test(itemClassName)
				&& !/zpMenu-item-hr/.test(itemClassName)) {
			tree.itemMouseHandler(item.__zp_item, Zapatec.Menu.MOUSEOVER);
		}
		item = tree && item.__zp_treeid
				? tree.items[item.__zp_item]
				: item.parentNode;
	}
...
Zapatec.Menu.onItemMouseOut = function() {
	...
	while (item && item != document.body) {
		...
		if (/zpMenu-item/.test(itemClassName)
				...
				!(/zpMenu-level-1/.test(itemClassName) && !/zpMenu-item-selected/
						.test(itemClassName))) {
	...
	if (!/zpMenuDisabled/.test(item.className)) {
		while (item && item != document.body) {
			...
			if (/zpMenu-item/.test(item.className)) {
				...
				// Show-on-click mode
				if (objMenu.config.onClick
						&& item.__zp_subtree
						&& (/zpMenu-top/
								.test(objMenu.trees[item.__zp_parent].className))) {
					objMenu
							.itemMouseHandler(item.__zp_item,
									Zapatec.Menu.CLICK);
					return Zapatec.Utils.stopEvent(ev);
				}
				...
				if (itemLink
					...
					if (objMenu.config.rememberPath
							|| objMenu.config.pathCookie != '__zp_item') {
						// Save path in cookies
						Zapatec.Utils.writeCookie(objMenu.config.pathCookie,
								item.__zp_item);
					}
					try {
						if (target) {
							...
							window.location.href = href; // may raise
															// exception in
															// Mozilla
						}
					} catch (e) {
 ...
 *            item_id Item ID
 * @param {number}
 *            type 0 = mouseout, 1 = mouseover, 2 = click
 */
Zapatec.Menu.prototype.itemMouseHandler = function(item_id, type) {
	...
	// animating subtree because current item may receive fake mouseout even
	// when
	// mouse pointer is over it and loose cursor. Other solution is needed
	// because
	...
	if (menu && menu.__zp_activeitem != item_id) {
		if (menu.__zp_activeitem) {
			...
			setTimeout(function() {
						...
						if (lastItem.__zp_state)
							objMenu.toggleItem(lastItem.__zp_item, false);
						Zapatec.Menu.selectItem(item);
					}, 0);
		} else {
			setTimeout(function() {
	...
	if (type == Zapatec.Menu.MOUSEOUT) {
		item.__zp_dimtimer = setTimeout(function() {
					...
					if (menu.__zp_activeitem == item_id)
	...
	if (this.config.onClick && !this.clickDone) {
		if (/zpMenu-top/.test(this.trees[item.__zp_parent].className)
				&& (type == Zapatec.Menu.MOUSEOVER))
			return;
		// Set the flag that enables further onmouseover activity.
		if (type == Zapatec.Menu.CLICK)
			this.clickDone = true;
	}

	// Setup show/hide timers.
	if (!item.__zp_state && type) {
		item.__zp_mousetimer = setTimeout('Zapatec.Menu.all["' + item.__zp_tree
						+ '"].itemShow("' + item.__zp_item + '")',
				(this.config.showDelay || 1));
	} else if (item.__zp_state && !type) {
		item.__zp_mousetimer = setTimeout('Zapatec.Menu.all["' + item.__zp_tree
						+ '"].itemHide("' + item.__zp_item + '")',
				(this.config.hideDelay || 1));
	}
};

/**
 * Called from the itemMouseHandler() after a timeout; positions and shows a
 * designated item's branch of the tree.
 * 
 * @private
 * @param {string}
 *            item_id Item ID to show
 */
Zapatec.Menu.prototype.itemShow = function(item_id) {
	...
	if (!subMenu.style.width || !subMenu.style.height) {
		...
		if (subMenu.offsetHeight > maxHeight) {
			// Need scrolling
			// iSubMenuOffsetHeight is needed because IE 5.0 delays with
			// offsetHeight
			...
			var funcMoveUp = function() {
				...
				if (objContainer.__zp_first.previousSibling != objUpArrow) {
					...
					if (iSpace > 0) {
						...
						var iItemHeight = iSpace
								- (objContainer.__zp_last.offsetHeight - objContainer.__zp_last.clientHeight);
						if (iItemHeight >= 0) {
							objContainer.__zp_last.style.display = 'none';
							objContainer.__zp_last.style.height = iItemHeight
									+ 'px';
			...
			var funcMoveDown = function() {
				...
				if (objContainer.__zp_last.nextSibling != objDownArrow) {
					...
					if (iSpace > 0) {
						...
						objContainer.__zp_first.style.display = 'block';
						var iItemHeight = iSpace
								- (objContainer.__zp_first.offsetHeight - objContainer.__zp_first.clientHeight);
						if (iItemHeight > 0) {
							objContainer.__zp_first.style.display = 'none';
							objContainer.__zp_first.style.height = iItemHeight
									+ 'px';
	...
	if ((/zpMenu-top/.test(this.trees[item.__zp_parent].className))
		...
		if (menuPos.x + newLeft + subMenu.offsetWidth + subMenuMarginLeft + 7 > scrollX
				+ winW) {
			newLeft += item.offsetWidth - subMenu.offsetWidth
					- subMenuMarginLeft;
			if (subMenu.__zp_dropshadow)
				newLeft -= 6;
		} else {
		...
		if (menuPos.y + newTop + subMenu.offsetHeight + subMenuMarginTop + 7 > scrollY
				+ winH) {
	...
	if (Zapatec.is_ie && !Zapatec.is_ie5) {
		...
		if (this.config.dropShadow) {
			Zapatec.Utils.setupWCH(subMenu.__zp_wch, -subMenuBorderLeft,
					-subMenuBorderTop, subMenu.offsetWidth + 6,
					subMenu.offsetHeight + 5);
		} else {
			Zapatec.Utils.setupWCH(subMenu.__zp_wch, -subMenuBorderLeft,
					-subMenuBorderTop, subMenu.offsetWidth,
 ...
 * </ul>
 * <script type="text/javascript"> var menu = new Zapatec.Menu('myMenu', {});
 * menu.itemDisable('itemToDisable'); </script> </xmp>
 * 
 * @param {string}
 *            item_id Item ID to disable
 */
 ...
 * @private
 * @param {number}
 *            iLeft Optional. Left position of trigger menu
 * @param {number}
 *            iTop Optional. Top position of trigger menu
 */
...
Zapatec.Menu.prototype.showMenu = function(iLeft, iTop) {
	...
	if (arguments.length > 1) {
		if (iLeft + objTopParent.offsetWidth > iScrollX + objWindowSize.width) {
			objTopParent.style.left = iScrollX + objWindowSize.width
					- objTopParent.offsetWidth + 'px';
		}
		if (iTop + objTopParent.offsetHeight > iScrollY + objWindowSize.height) {
			objTopParent.style.top = iScrollY + objWindowSize.height
					- objTopParent.offsetHeight + 'px';
		}
	} else {
		if (typeof this.config.top != 'object'
				|| typeof this.config.right != 'object'
				|| typeof this.config.bottom != 'object'
				|| typeof this.config.left != 'object') { // null is of object
															// type
			objTopParent.style.position = 'absolute';
			if (typeof this.config.top != 'object') {
				objTopParent.style.top = parseInt(this.config.top) + 'px';
			} else if (typeof this.config.bottom != 'object') {
				objTopParent.style.top = (objWindowSize.height
						- parseInt(this.config.bottom) - objMenu.offsetHeight - (objTopParent.offsetHeight - objTopParent.clientHeight))
						+ 'px';
			}
			if (typeof this.config.left != 'object') {
				...
				objTopParent.style.left = (objWindowSize.width
						- parseInt(this.config.right) - objMenu.offsetWidth - (objTopParent.offsetWidth - objTopParent.clientWidth))
						+ 'px';
			}
		} else if (window.opera
				&& (this.config.drag || this.config.scrollWithWindow)) {
			objTopParent.style.position = 'absolute';
			var pos = Zapatec.Utils.getAbsolutePos(objTopParent);
	...
	if (Zapatec.is_ie && !Zapatec.is_ie5) {
		...
		Zapatec.Utils.setupWCH(objTopParent.__zp_wch, -objTopParent.clientLeft,
				-objTopParent.clientTop, objTopParent.offsetWidth,
				objTopParent.offsetHeight);
	}
};
