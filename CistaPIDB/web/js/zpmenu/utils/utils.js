 ...
 * 
 * Copyright (c) 2004-2006 by Zapatec, Inc. http://www.zapatec.com 1700 MLK Way,
 * Berkeley, California, 94709, U.S.A. All rights reserved. $Id: utils.js 4131
 * 2006-08-02 09:19:18Z slip $
 * 
 * 
...
// / This function is not actually very useful, but it's legacy from the old
// calendar code.
// / @param ev [Event] the event object.
// / @return [HTMLElement] window.event.srcElement for MSIE, ev.currentTarget
// for other browsers.
...
// calendar code.
// / @param ev [Event] the event object.
// / @return [HTMLElement] window.event.srcElement for MSIE, ev.target for other
// browsers.
 ...
 * 
 * @param {object}
 *            objEvent Optional. Event object
...
// / @param selectable [boolean] the flag to indicate wether element is
// selectable(rather usefull).
...
Zapatec.Utils.createElement = function(type, parent, selectable) {
	...
	if (window.self.document.createElementNS)
		...
		el = window.self.document.createElementNS(
 ...
 * @return [string or null] a string with the cookie value, or null if it can't
 *         be found.
 */

/*
 * ? inside regular expression is not supported in IE 5.0
 * Zapatec.Utils.getCookie = function(name) { var re = new RegExp("(^|;\\s*)" +
 * name + "\\s*=(.*?)(;|$)"); if (re.test(document.cookie)) { var value =
 * RegExp.$2; value = unescape(value); return (value); } return null; };
 */
 ...
 * 
 * @param obj
 *            [Object] reference to the object to serialize.
...
/**
 * The reverse of Zapatec.Utils.makePref(), this function unserializes the given
 * string and creates an object from it.
 * 
 * @param txt
 *            [string] the serialized value.
 * @return [Object] a new object if it was created successfully or null
 *         otherwise.
 */
...
/**
 * Create an WCH object. This function does nothing if the browser is not IE5.5
 * or IE6.0. A WCH object is one of the most bizarre tricks to avoid a notorious
 * IE bug: IE normally shows "windowed controls" on top of any HTML elements,
 * regardless of any z-index that might be specified in CSS. This technique is
 * described at: http://www.aplus.co.yu/WCH/
 * 
 ...
 * 
 * @param element
 *            [HTMLElement, optional] -- Create the WCH IFRAME inside this.
 * 
 * 
 * @return [HTMLIFrame or null] a new WCH object if the browser is "supported",
 *         null otherwise.
 */
		...
		element
				.insertAdjacentHTML(
						...
						'<iframe id="'
								...
								+ 'style="z-index:0;position:absolute;visibility:hidden;'
								+ filter
 ...
 * 
 * @param f
 *            [HTMLIFrame] the WCH object
 * @param el
 *            [HTMLElement] the element to cover.
 * @param el2
 *            [HTMLElement, optional] another element to cover.
 */
Zapatec.Utils.setupWCH_el = function(f, el, el2) {
	if (f) {
		var pos = Zapatec.Utils.getAbsolutePos(el), X1 = pos.x, Y1 = pos.y, X2 = X1
				+ el.offsetWidth, Y2 = Y1 + el.offsetHeight;
		if (el2) {
			var p2 = Zapatec.Utils.getAbsolutePos(el2), XX1 = p2.x, YY1 = p2.y, XX2 = XX1
					+ el2.offsetWidth, YY2 = YY1 + el2.offsetHeight;
			if (X1 > XX1)
				X1 = XX1;
			if (Y1 > YY1)
				Y1 = YY1;
...
/**
 * Registers a given object to have its style.top set equal to the window scroll
 * position as the browser scrolls.
 * 
 * @param node
 *            [HTMLElement] -- a reference to the node to scroll.
 */
 ...
 * 
 * @param newScrollY
 *            [number] -- the new window scroll position.
 * @param direction
 *            [string] - "vertical"/"horizontal" controlls which scrolling we
 *            are handling
 */
Zapatec.ScrollWithWindow.handler = function(newScroll, direction) {
	// Move oldScrollY towards newScrollY, evening up if the difference is
	// small.
	if (direction == "vertical") {
		var newScrollY = newScroll;
		oldScrollY += ((newScrollY - oldScrollY) * this.stickiness);
		if (Math.abs(oldScrollY - newScrollY) <= 1)
			oldScrollY = newScrollY;
	} else {
		var newScrollX = newScroll;
		oldScrollX += ((newScrollX - oldScrollX) * this.stickiness);
		if (Math.abs(oldScrollX - newScrollX) <= 1)
...
/**
 * Destroys the given element (remove it from the DOM tree) if it's not null and
 * it's parent is not null.
 * 
 * @param el
 *            [HTMLElement] the element to destroy.
 */
 ...
 * 
 * @param url
 *            [string] the URL to open a new window to.
 * @param windowName
 *            [string] the name of the new window (as for target attribute).
 * @param width
 *            [number] the width of the new window in pixels.
 * @param height
 *            [number] the height of the new window in pixels.
 * @param scrollbars
 *            [string] "yes" or "no" for scrollbars.
 * 
 * @return [object] the new window
 */
Zapatec.Utils.newCenteredWindow = function(url, windowName, width, height,
		scrollbars) {
	var leftPosition = 0;
	var topPosition = 0;
	if (screen.width)
	...
	var winArgs = 'height=' + height + ',width=' + width + ',top='
...
/**
 * Finds the size of the current web page. This is the usable size and does not
 ...
 * 
 * @param sel
 *            [HTMLSelectElement] reference to the SELECT element.
 * @param val
 *            [string] the value that we should select.
 * @param call_default
 *            [boolean] true if the default onchange should be called.
 */
...
/**
 * A more flexible way to get the "nextSibling" of a given element. If the "tag"
 * argument is passed, then this function will return the next sibling that has
 * a certain tag. Otherwise it will simply return el.nextSibling.
 * 
 * @param el
 *            [HTMLElement] reference to the anchor element.
 * @param tag
 *            [string] the tag name of the returned node.
 * @param alternateTag
 *            [string] the alternate tag name of the returned node.
 * 
 * @return [HTMLElement or null] el.nextSibling if tag is not passed, or the
 *         first element after el having the specified tag. Null is returned if
...
Zapatec.Utils.getNextSibling = function(el, tag, alternateTag) {
	...
	while (el) {
		if (el.nodeType == 1
				&& (el.tagName.toLowerCase() == tag || (alternateTag && el.tagName
						.toLowerCase() == alternateTag))) {
			return el;
		}
		el = el.nextSibling;
	}
...
/**
 * Function that concatenates and returns all text child nodes of the specified
 * node.
 * 
 * @param objNode
...
/**
 * Similar to the DOM's built in insertBefore. Insert a node after an existing
 * node.
 * 
 * @param el
 *            [oldNode] The existing element
 * @param el
 *            [newNode] the new element to insert after the old one.
 * 
 */
...
Zapatec.Utils._ids = {};
/** < [number, static] maintains a list of generated IDs */
 ...
 * 
 * @param code
 *            [string] the class of ids. User defined, can be anything.
 * @param id
 *            [string, optional] specify if the ID is already known.
 * 
 ...
 * "title" attribute set; if so, its value will be displayed highlighted as the
 * title of the tooltip.
 * 
 * @param target
 *            reference to or ID of the target element
 * @param tooltip
 *            reference to or ID of the tooltip content element
 */
...
Zapatec.Utils.checkActivation = function() {
	...
	for (i in arrProducts)
		...
		if (arrProducts[i].bActive == true)
			strMsg += '\nTo purchase the Zapatec ' + arrProducts[i].name
					+ ' visit www.zapatec.com/website/main/products/'
					+ arrProducts[i].webdir + '/'
...
if (!Array.prototype.splice) {
	Array.prototype.splice = function(index, howMany) {
		...
		for (i = this.length + elements.length - 1; i >= index
				+ elements.length; i--) {
 ...
 * 
 * \param objArgs [number] error object: { severity: [string, optional] error
 * severity, description: [string] human readable error description }
 */
...
/**
 * Inserts the element into array. It influences the order in which the elements
 * will be iterated in the for...in cycle.
 * 
 * @param arr
 *            [array] array to work with.
 * @param el
 *            [mixed] element to insert.
 * @param key
 *            [string] element to insert.
 * @param nextKey
 *            [string] element to be inserted before.
 ...
 *  // Define SuperClass and its methods Zapatec.SuperClass = function(objArgs) {
 * ... };
 * 
 * Zapatec.SuperClass.prototype.init = function(objArgs) { ... };
 *  // Define SubClass and its methods Zapatec.SubClass = function(objArgs) {
 * ... // Call constructor of superclass
 * Zapatec.SubClass.SUPERconstructor.call(this, objArgs); ... };
 *  // Inherit SuperClass Zapatec.inherit(Zapatec.SubClass, Zapatec.SuperClass);
 * 
 * Zapatec.SubClass.prototype.init = function(objArgs) { ... // Call method of
 * superclass Zapatec.SubClass.SUPERclass.init.call(this, objArgs); ... };
 * 
 ...
 * correctly.
 * 
 * \param objSubClass [object] inheriting class. \param objSuperClass [object]
...
Zapatec.getPath = function() {
	...
	if (objScript.nodeType != 1 || objScript.nodeName.toLowerCase() != 'script') {
		return "";
	}
...
/*
 * Use this method to display your custom message before unload event. For
 * example you could warn user that he has some unsaved changes on page.
 * 
 * \param msg [string] - message to display. Default value - "All your changes
 * will be lost.": \param win [object] - reference to window object. By default -
 * current window
 */
...
/**
 * Assigns a value to a custom property of DOM object. This property will be set
 * to null on window unload event. Use this function to create properties that
 * can't be destroyed by garbage collector due to circular references.
 * 
 * @param {object}
 *            objElement DOM object
 * @param {string}
...
Zapatec.Utils.addEvent(window, 'unload', function() {
	...
	for (var iLis = Zapatec.Utils.removeOnUnload.length - 1; iLis >= 0; iLis--) {
		...
		Zapatec.Utils.removeEvent(listener["element"], listener["event"],
				listener["listener"]);
	}
});
