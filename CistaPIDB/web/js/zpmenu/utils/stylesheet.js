/**
 * \file stylesheet.js Zapatec StyleSheet class definition. Used to manipulate
 * with style sheets.
 * 
 ...
 * 
 * \param strSelector [string] rule selector. \param strDeclarations [string]
 * rule declarations.
 */
Zapatec.StyleSheet.prototype.addRule = function(strSelector, strDeclarations) {
	if (document.createStyleSheet) {
		...
		/*
		 * Commenting for now because works properly only in Firefox } else if
		 * (document.styleSheets) { // Firefox, Safari, Konqueror var
		 * objStyleSheet = document.styleSheets.item(this.n);
		 * objStyleSheet.insertRule(strSelector + ' { ' + strDeclarations + '
		 * }', objStyleSheet.cssRules.length);
		 */
	} else {
		// Opera
		this.styleSheet.appendChild(document.createTextNode(strSelector + ' { '
						+ strDeclarations + ' }'));
	}
};
...
Zapatec.StyleSheet.prototype.removeRules = function() {
	if (document.createStyleSheet) {
		 ...
		 * Commenting for now because works properly only in Firefox } else if
		 * (document.styleSheets) { // Firefox, Safari, Konqueror var
		 * objStyleSheet = document.styleSheets.item(this.n); var iRules =
		 * objStyleSheet.cssRules.length; for (var iRule = 0; iRule < iRules;
		 * iRule++) { objStyleSheet.deleteRule(0); }
		 */
	} else {
		// Opera
