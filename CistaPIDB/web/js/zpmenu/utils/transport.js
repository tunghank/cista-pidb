...
if (typeof ActiveXObject != 'undefined') {
	...
	/**
	 * @ignore Returns first available ActiveX object name from the given list.
	 * 
	 * @param {object}
	 *            arrVersions List of ActiveX object names to test
...
Zapatec.Transport.showBusy = function(objArgs) {
	...
	if (typeof strImage != 'string' || !strImage.length) {
		...
		if (typeof strImageWidth == 'number'
				|| (typeof strImageWidth == 'string' && /\d$/
						.test(strImageWidth))) {
			strImageWidth += 'px';
		}
		if (typeof strImageHeight == 'number'
				|| (typeof strImageHeight == 'string' && /\d$/
						.test(strImageHeight))) {
			strImageHeight += 'px';
		}
	}
...
Zapatec.Transport.fetch = function(objArgs) {
	...
	// IE 6 calls onreadystatechange and then raises an exception if local file
	// is
	...
	try {
		...
		if (!boolErrorDisplayed) {
			...
			if (objException.name
				...
				Zapatec.Transport.displayError(0, 'Error: Cannot fetch '
								+ objArgs.url + '.\nFile not found.',
						objArgs.onError);
			} else {
				Zapatec.Transport
						.displayError(0, 'Error: Cannot fetch ' + objArgs.url
...
Zapatec.Transport.parseHtml = function(strHtml) {
	// Convert to string
	strHtml += '';
	// Remove leading whitespace characters because Firefox and Opera don't
	// parse
	...
	if (document.createElementNS) {
		...
		objTempContainer = document.createElementNS(
 ...
 * 
 * <p>
 * <b> Note: Global variables must be declared without "var" keyword. Otherwise
 * they will be ignored by Safari. </b>
 * </p>
 * 
 * @param {string}
 *            strScript Script to evaluate
 */
Zapatec.Transport.evalGlobalScope = function(strScript) {
	...
	if (window.execScript) {
		...
		/*
		 * This should never be reached } else { var funcScript = new
		 * Function(strScript); funcScript.call(window);
		 */
	}
};
 ...
 * 
 * <p>
 * <b> Note: Scripts are executed after HTML fragment is assigned to innerHTML.
 * If external scripts are used, they are loaded asynchronously and execution
 * sequence is not preserved. </b>
 * </p>
 * 
 * <p>
 * <b> Note: Global variables must be declared without "var" keyword. Otherwise
...
Zapatec.Transport.setInnerHtml = function(objArgs) {
			...
			if (strToken.match(/\S/)) {
				...
				if (arrMatch) {
					...
					if (strAttrs.match(/\s+src\s*=/i)) {
						...
						srtScript = srtScript.replace(/function\s+([^(]+)/g,
								'$1=function');
					}
	...
	for (var iScript = 0; iScript < arrScripts.length; iScript++) {
		...
		strAttrs = strAttrs.replace(/\s+/g, ' ').replace(/^\s/, '').replace(
				/\s$/, '').replace(/ = /g, '=');
...
Zapatec.Transport.fetchXmlDoc = function(objArgs) {
	...
	if (!objArgs.method && typeof objArgs.username == 'undefined'
		...
		if (document.implementation && document.implementation.createDocument) {
			// Mozilla
			var objDocument = document.implementation.createDocument('', '',
					null);
			// Opera 8.51 also has document.implementation, but hasn't
			// implemented
			...
			if (objDocument.load) {
				...
				if (objArgs.async) {
					objDocument.onload = function() {
						...
						Zapatec.Transport.removeBusy(objArgs);
						// Process response
						Zapatec.Transport.onXmlDocLoad(objDocument,
								objArgs.onLoad, objArgs.onError);
					};
				}
				// Show "Busy" animated GIF
				Zapatec.Transport.showBusy(objArgs);
				...
				try {
					...
					if (objException.name
							&& objException.name == 'NS_ERROR_FILE_NOT_FOUND') {
						Zapatec.Transport
								.displayError(0, 'Error: Cannot fetch '
												+ objArgs.url
												+ '.\nFile not found.',
										objArgs.onError);
					} else {
 ...
 * @private
 * @param {object}
 *            objDocument XMLDocument object
 * @param {function}
 *            onLoad Callback function provided by user
 * @param {function}
 *            onError Callback function provided by user
 */
Zapatec.Transport.onXmlDocLoad = function(objDocument, onLoad, onError) {
	...
	if (objDocument.parseError) {
		...
		if (objDocument.parseError.srcText) {
			strError += 'Location: ' + objDocument.parseError.url
					+ '\nLine number ' + objDocument.parseError.line
					+ ', column ' + objDocument.parseError.linepos + ':\n'
					+ objDocument.parseError.srcText + '\n';
		}
	} else if (objDocument.documentElement
			&& objDocument.documentElement.tagName == 'parsererror') {
		// If an error is caused while parsing, Mozilla doesn't throw an
		// exception.
		// Instead, it creates an XML string containing the details of the
		// error:
	...
	if (strError) {
		// Parsing error
		Zapatec.Transport.displayError(0, 'Error: Cannot parse.\n' + strError,
				onError);
	} else {
		// Success
		...
		Zapatec.Transport
				.displayError(
						...
						'Error: Cannot parse.\n'
								...
								+ objException.message + '\n'
								+ objException.text, objArgs.onError);
	};
	if (typeof objArgs.onLoad == 'function') {
 ...
 * @private
 * @param {string}
 *            strUrl Absolute or relative URL of JS file
 * @param {string}
 *            strAbsoluteUrl Optional. Absolute URL of JS file
...
Zapatec.Transport.isLoadedJS = function(strUrl, strAbsoluteUrl) {
	...
	if (typeof strAbsoluteUrl == 'undefined') {
		strAbsoluteUrl = Zapatec.Transport.translateUrl({
					url : strUrl
				});
	}
 ...
 * 
 * @param {string}
 *            strScriptFileName Script file name, e.g. 'zpmywidget.js'
 ...
 * 
 * @param {string}
 *            strSrc Src attribute value of the script element
 * @param {boolean}
 *            boolForce Optional. Force reload if it is already loaded
 */
Zapatec.Transport.include = function(strSrc, boolForce) {
	// Get absolute URL of the JS file
	var strAbsoluteUrl = Zapatec.Transport.translateUrl({
				url : strSrc
			});
...
/**
 * Includes JS file into the page. Allows URLs from foreign domains. Notice that
 ...
 * 
 * @param {string}
 *            strHref Href attribute value of the link element
 */
			...
			Zapatec.Transport.fetch({
				...
				onLoad : function(objRequest) {
					...
					while (iNextPos >= 0) {
						...
						// Move second cursor to the new location to start the
						// search from
						iPos = iNextPos;
						...
						if (iNextPos >= 0) {
							...
							// Move second cursor to the new location to start
							// the search from
							iPos = iNextPos;
							// Search next pattern
