/**
 * @fileoverview Javascript Logger (in the spirit of log4j) This library is
 *               designed to make the writing and debugging of javascript code
 *               easier, by allowing the programmer to perform debug or log
 *               output at any place in their code. This supports the concept of
 *               different levels of logging (debug < info < warn < error <
 *               fatal << none) as well as different log outputs. Three log
 *               outputs are included, but you can add your own. The included
 *               log outputs are {@link Log#writeLogger},
 *               {@link Log#alertLogger}, and {@link Log#popupLogger}. For
 *               debugging on Safari, the log ouput {@link Log#consoleLogger} is
 *               also included. To turn off debugging but still leave the logger
 *               calls in your script, use the log level {@link Log#NONE}.
 * 
 * Example usage:
 * 
 * <pre>
 ...
 * 
 * @constructor
 * @class The main Log class. Create a new instance of this class to send all
 *        logging events.
 * @param level
 *            The cut-off logger level. You can adjust this level in the
 *            constructor and leave all other logging events in place. Defaults
 *            to {@link Log#WARN}.
 * @param logger
 *            The logger to use. The logger is a function that accepts the
 *            logging events and informs the user or developer. Defaults to
 *            {@link Log#writeLogger}.
 */
function Log(level, logger, prefix) {
	 ...
	 * 
	 * @param logger
	 *            The function that will be called when a log event needs to be
	 *            displayed
	 */
	...
	/**
	 * Sets the current threshold log level for this Log instance. Only events
	 * that have a priority of this level or greater are logged.
	 * 
	 * @param level
	 *            The new threshold priority level for logging events. This can
	 *            be one of the static members {@link Log#DEBUG},
	 *            {@link Log#INFO}, {@link Log#WARN}, {@link Log#ERROR},
	 *            {@link Log#FATAL}, {@link Log#NONE}, or it can be one of the
	 *            strings ["debug", "info", "warn", "error", "fatal", "none"].
	 */
...
/**
 * _log is the function that actually calling the configured logger function. It
 * is possible that this function could be extended to allow for more than one
 * logger.
 * 
 * This method is used by {@link Log#debug}, {@link Log#info},
 * {@link Log#warn}, {@link Log#error}, and {@link Log#fatal}
 * 
 * @private
 * @param {String}
 ...
 * @param {Log}
 *            obj The originating {@link Log} object.
 */
...
/**
 * Static alert logger method. This logger will display a javascript alert
 * (messagebox) with the message.
 * 
 * @param {String}
 *            msg The message to display
 * @param level
...
/**
 * Static write logger method. This logger will print the message out to the web
...
/**
 * Static Safari WebKit console logger method. This logger will write messages
 * to the Safari javascript console, if available. If this browser doesn't have
 * a javascript console (IE/Moz), then it degrades gracefully to
 * {@link Log#popupLogger}
 * 
 * @param {String}
 *            msg The message to display
 * @param level
...
/**
 * Static popup logger method. This logger will popup a new window (if
 * necessary), and add the log message to the top of a list.
 * 
 * @param {String}
 *            msg The message to display
 * @param level
...
Log.popupLogger = function(msg, level, obj) {
	...
	if (!obj._window || !obj._window.document) {
		obj._window = window
				.open("", 'logger_popup_window',
						'width=420,height=320,scrollbars=1,status=0,toolbars=0,resizeable=1');
		if (!obj._window) {
		...
		if (!obj._window.document.getElementById('loggerTable')) {
			obj._window.document
					.writeln("<table width='100%' id='loggerTable'><tr><th align='left'>Time</th><th width='100%' colspan='2' align='left'>Message</th></tr></table>");
...
/**
 * This method is a utility function that takes an object and creates a string
 * representation of it's members.
 * 
 * @param {Object}
 ...
 * 
 * @param {Object}
 *            the Object to tune the dump of an object
 */
...
/*
 * Recursive function to dump the object @param {obj} - object to dump @param
 * {name} - name of object @param {configIn} - object to tune the dump of this
 * object
 */
