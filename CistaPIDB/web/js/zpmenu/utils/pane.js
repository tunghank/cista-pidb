/**
 * Zapatec Pane object. Creates the element for displaying content and gives an
 ...
 * -------------------------------------------------------------------------------------------------
 * parent | [string or object] Reference to DOM element where | newly created
 * Pane will be placed. By default - | document.body containerType | [string]
 * Required. Possible values: div|iframe|current - | what type of pane to create | *
 * div - create new DIV element and add it into parent | * iframe - create new
 * IFRAME element and add it into parent | * current - use parent as pane
 * container sourceType | [string] see Zapatec.Widget documentation for this
 * option source | [string or object] see Zapatec.Widget documentation | for
 * this option \endcode
 */
...
Zapatec.Pane.prototype.init = function(objArgs) {
	...
	if (this.config.parent == null) {
		Zapatec.Log({
					description : "No reference to parent element."
				})
		return null;
	}
	...
	if (this.config.containerType.toLowerCase() == 'iframe') {
		...
		Zapatec.Log({
					description : "Unknown container type: "
							+ this.config.containerType
							+ ". Possible values: iframe|div"
				})
	}
...
Zapatec.Pane.prototype.initPane = function() {
	if (this.config.containerType.toLowerCase() == 'iframe') {
		var doc = null;
		// this variable determines if the Pane has the content from the same
		// domain
		var sameDomain = true;
		// iframe's src
		...
		if (protocolSeparatorPos != -1) {
			...
			var domain = url.substring((protocolSeparatorPos > 0
							? protocolSeparatorPos + 3
							: 0), (domainSeparatorPos > 0
							? domainSeparatorPos
							: url.length));
		...
		if (sameDomain) {
			...
			} else if (this.container.contentWindow
					&& this.container.contentWindow.document != null) {
...
/**
 * Loads data from the HTML source. \param objSource [object] source HTMLElement
 * object.
 */
	...
	if (content == null) {
		...
		if (typeof(content) == 'string') {
			...
			try {
				...
				// this is temporary fix for IE as it can not appendChild in
				// iframe
		...
		// if this is not an iframe we get new sizes and set them as the size of
		// Pane
		if (this.config.containerType.toLowerCase() != "iframe") {
			var newWidth = this.getWidth();
			var newHeight = this.getHeight();
		} else {
			// if this is iframe we are using scrollWidth and scrollHeight to
			// resize iframe.
			// FIXME: This is not working correctly so needed to be fixed in new
			// version
...
/**
 * Loads data from the HTML|Xml fragment source. \param strSource [string]
...
/*
 * Set pane width \param width [int] - width to set.
 */
Zapatec.Pane.prototype.setWidth = function(width) {
	...
	this.fireWhenReady(function() {
				...
				if (self.getContainer().offsetWidth != width) {
					var newWidth = width
							- (self.getContainer().offsetWidth - width);
					if (newWidth < 0)
						newWidth = 0;
...
/*
 * Set pane height \param height [int] - height to set.
 */
	...
	this.fireWhenReady(function() {
				...
				if (self.getContainer().offsetHeight != height) {
					var newHeight = height
							- (self.getContainer().offsetHeight - height);
					if (newHeight < 0)
						newHeight = 0;
...
/*
 * The method to cover setting of content. Simply it sets the content depending
 * on its type. @param content [mixed] - value for the content @param type
 * [string] - type of content: "html", "html/text", "html/url" @return [boolean] -
...
/*
 * Fires the action when widget is ready. @param func [function] - the function
 * to be fired when the widget is ready
 */
...
/**
 * Zapatec SplitPane object. Creates the element for displaying vertical or
 * horizontal row of Zapatec.Pane objects with possibility of dynamic resizing.
 * 
 * @param config
 *            [object] - SplitPane config.
 * 
 * Constructor recognizes the following properties of the config object \code
 * property name | description
 * -------------------------------------------------------------------------------------------------
 * parent | [string or object] Reference to DOM element where | newly created
 * Pane will be placed. By default - | document.body containerType | [string]
 * Required. Possible values: div|iframe - | what type of pane to create
 * sourceType | [string] see Zapatec.Widget documentation for this option source |
 * [string or object] see Zapatec.Widget documentation | for this option
 * \endcode
 */
...
Zapatec.SplitPane.prototype.init = function(objArgs) {
	...
	this.container.className = this.getClassName({
		prefix : "zpSplitPane",
		suffix : "Container"
				+ (this.config.orientation.charAt(0).toUpperCase() + this.config.orientation
						.substring(1))
	});
