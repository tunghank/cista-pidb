 ...
 *            strEvent Event name
 * @param {function}
 *            funcListener Event listener
 * @param {function}
 *            first If true funcListener will be put as the first one
 */
...
Zapatec.Widget.prototype.init = function(objArgs) {
	...
	if (typeof this.constructor.path != 'undefined') {
		this.defineConfigOption('themePath', this.constructor.path
						+ '../themes/');
	} else {
 ...
 *          of new config option. If default value is not specified, it is set
 *          to null.
 * 
 * @param {string}
 *            strOption Config option name
 * @param {any}
 *            val Optional. Config option default value
 */
...
Zapatec.Widget.prototype.addUserEventListeners = function() {
	for (var strEvent in this.config.eventListeners) {
		if (this.config.eventListeners.hasOwnProperty(strEvent)) {
			this.addEventListener(strEvent,
					this.config.eventListeners[strEvent]);
		}
	}
};
...
Zapatec.Widget.prototype.addStandardEventListeners = function() {
	this.addEventListener('loadThemeError', function(objError) {
				Zapatec.Log({
							description : 'Can not load theme. '
...
Zapatec.Widget.prototype.loadTheme = function() {
	...
	if (this.config.theme) {
		...
		Zapatec.Transport.loadCss({
					// URL of theme file
					url : strUrl,
					// Suspend script execution until theme is loaded or error
					// received
	...
	if (this.container != null && this.config.theme && !this.themeLoaded) {
		...
		if (typeof(Zapatec.Progress) == 'undefined') {
			...
			Zapatec.Transport.loadJS({
						module : 'progress',
						onLoad : function() {
							// if theme is already loaded - do not show progress
							// bar
							if (self.themeLoaded) {
								return null;
							}
		...
		this.loader = new Zapatec.Progress({
					...
					themePath : Zapatec.Progress.path
							+ "../../zpextra/themes/progress/"
				});
...
/**
 * @private Hides progressbar created using #showLoader and shows widget. This
 *          method will be called automatically on theme loading.
 */
 ...
 * @param {number}
 *            animSpeed possible values - 1..100. Bigger value - more fast
 *            animation.
 * @param {function}
 *            onFinish Function to call on effect end.
 */
 ...
 *            animation.
 * @param {boolean}
 *            show if true - show widget. Otherwise - hide.
 */
Zapatec.Widget.prototype.showHideContainer = function(effects, animSpeed,
		onFinish, show) {
	...
	if (typeof(Zapatec.Effects) == 'undefined') {
		...
		Zapatec.Transport.loadJS({
					url : Zapatec.Widget.path
							+ '../../zpeffects/src/effects.js',
					onLoad : function() {
						self.showHideContainer(effects, animSpeed, onFinish,
								show);
					}
				});
 ...
 * <ul>
 * <li>
 * &lt;i&gt;fetchSourceStart&lt;/i&gt; when Zapatec.Transport#fetch is called
 * </li>
 * <li>
 * &lt;i&gt;fetchSourceEnd&lt;/i&gt; when Zapatec.Transport#fetch returns result or
 *  error
 * </li>
 * <li>
 * &lt;i&gt;fetchSourceError&lt;/i&gt; when Zapatec.Transport#fetch returns error.
 ...
 * <ul>
 * <li>
 * &lt;i&gt;loadDataStart&lt;/i&gt; when data parsing is started
 * </li>
 * <li>
 * &lt;i&gt;loadDataEnd&lt;/i&gt; when data parsing is ended or error occured during
 *  fetch
 * </li>
 * </ul>
 * </pre>
 * 
 * @param {object}
 *            objArgs Arguments object passed to callbackSource function
 */
Zapatec.Widget.prototype.loadData = function(objArgs) {
	...
	if (this.config.source != null && this.config.sourceType != null) {
		...
		if (strSourceType == 'html') {
			...
			this
					.loadDataHtml(Zapatec.Widget
							.getElementById(this.config.source));
			...
			Zapatec.Transport.fetch({
						...
						// Suspend script execution until source is loaded or
						// error received
 ...
 * 
 * @param {string}
 *            strSource Source HTML fragment
 */
 ...
 * 
 * @param {object}
 *            objSource Source XMLDocument object
 */
 ...
 * Event object when event handler is set using element attribute, e.g. &lt;a
 * onclick="eventHandler()"&gt;.
 * 
 * @param {object}
 *            arrEventNames array of event names where "window.event" is needed,
 *            e.g. ['click', 'dblclick'].
 */
Zapatec.Widget.emulateWindowEvent = function(arrEventNames) {
	if (document.addEventListener) {
		...
		for (var iEvent = 0; iEvent < arrEventNames.length; iEvent++) {
			document.addEventListener(arrEventNames[iEvent],
					function(objEvent) {
						if (objEvent) {
