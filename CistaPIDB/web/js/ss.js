...
/**
 * Smart Search Setup: param. name | description
 * -------------Required------------------------------------------------------------------------------
 * button | The smart search button's id. (Required) inputField | Input text
 * field. (Required) cp | Web application context path. (Required)
 * -------------Optional(Must)------------------------------------------------------------------------
 * name | The pre-defined smart search name. Mapping to the name defined in
 * SmartSearch.xml. (Required if 'table' not specified) | table | The database
 * table name for query. (Required if 'name' not specified) keyColumn | The
 * table's column name for query. (Required if 'table' is specified) columns |
 * The columns for displaying (returning), split by comma (,). (Required if
 * 'table' is specified)
 * -------------Optional------------------------------------------------------------------------------
 * title | The title displayed on smart search dialog. mode | Special smart
 * search mode: 0=single select, 1=multi select. whereCause | The optional sql
 * where cause statment. Support parameters ex: USER_NAME = {userName} and
 * ACTIVE = 1. orderBy | The order column name. Default order by key column.
 * callbackHandle| The javascript function name for smart search call back, when
 * selection completed in smart search dialog. autoSearch | Whether to carry
 * parameter in text field, and execute auto search. true or false. Usually true
 * on query page, false on create/edit page. width | The dialog width. height |
 * The dialog height.
 * 
 */
SmartSearch.setup = function(params) {
	...
	if (params.button == null
			|| params.inputField == null
			|| params.cp == null
			|| (params.name == null && !(params.table != null && params.keyColumn != null))) {
	...
	params.button["onclick"] = function() {
		...
		var x = window.open(target, "smart_search",
				"scrollbar=auto, help=false, resizable=false, status=false,height="
						+ height + "px,width=" + width + "px,left=" + left
						+ "px,top=" + top + "px");
		x.focus();
	}
};
...
function onSSSelect() {
	if (vHandle != "") {
		...
		if (selected.length > 0) {
			eval("window.opener." + vHandle
					+ "(vInputField, vColumns, selected)");
		}
	}
	window.close();
}
...
function SSSelectCallbackHandle(selectField, columns, value) {
	if ($(selectField) && value != null && value.length > 0) {
		...
		for (var i = 0; i < value.length; i++) {
			addOption($(selectField), value[i][columns[0]],
					value[i][columns[0]]);
		}
	}
}
