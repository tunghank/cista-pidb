...
function selectPocketQua() {
	...
	var target = "<%=cp%>/dialog/select_rw_pocketQua.do?m=list&callback=selectPocketQuaComplete";
	var width = 600;
	var height = 400;
	var left = (screen.width-width)/2;
	var top = (screen.height-height)/2;
	var x = window.open(target,"select_rw_pocketQua","scrollbar=auto, help=false, resizable=false, status=false,height="+height+"px,width="+width+"px,left="+left+"px,top="+top+"px");
	x.focus();
}
...
function selectTrayDrawingNo() {
	...
	var target = "<%=cp%>/dialog/select_rw_trayDrawingNo.do?m=list&callback=selectTrayDrawingNoComplete&trayDrawingNo="+$F('trayDrawingNo');
	var width = 600;
	var height = 400;
	var left = (screen.width-width)/2;
	var top = (screen.height-height)/2;
	var x = window.open(target,"select_rw_trayDrawingNo","scrollbar=auto, help=false, resizable=false, status=false,height="+height+"px,width="+width+"px,left="+left+"px,top="+top+"px");
	x.focus();
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="pageTitle">Master Data :: RW Query</td>
  </tr>
</table>
			<div class="content">
			<form id="rwQueryForm" action="<%=cp %>/md/rw_query.do?m=query" method="post">
