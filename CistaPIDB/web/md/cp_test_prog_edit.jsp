...
function doSave(s) {
	...
	if (submitForm ) {
		...
		$('cpTestProgEdit').action = $('cpTestProgEdit').action + "?m=save&toErp=" + $F('toErp');
		$('cpTestProgEdit').submit();	
	}
}
...
function findMaterialDescNum() {
	//alert($('materialNum').value);
	if($('materialNum').value !=""){
		...
		{
			method: 'post',
			parameters: 'projWver=' + $('projCodeWVersion').value+'&materialNum='+$('materialNum').value
				+'&cpMaterialNum='+$('cpMaterialNum').value,
...
<body  onLoad="findMaterialDescNum();">
<form name="cpTestProgEdit" action="<%=cp %>/md/cp_test_prog_edit.do" method="post">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="pageTitle">Master Data :: Modify CP Test Program</td>
  </tr>
</table>
	<div class="content">
    ...
    <tbody>
      <tr>
			...
			<td><input name="cpMaterialNum" readonly type="text"
							class="text_protected" id="cpMaterialNum"
							value="<%=BeanHelper.getHtmlValueByColumn(ref, "CP_MATERIAL_NUM") %>">					
	 </tr>
      <tr>
			<th width="180"><div class="erp_label"><span class="star">*</span>Project Code</div></th>
			<td><input name="projCode" readonly type="text"
							class="text_protected" id="projCode"
							value="<%=BeanHelper.getHtmlValueByColumn(ref, "PROJ_CODE") %>">					
	 </tr>
					<tr>
						<th width="180"><span class="star">*</span>Product Code</th>
						<td><input name="prodCode" type="text"
							class="text_protected" id="prodCode" readonly
					...
					<tr>
						...
						<td><input name="prodName" type="text"
							class="text_protected" readonly id="prodName"
					...
					<tr>
						...
						<td><input name="projCodeWVersion" type="text"
							class="text_protected" id="projCodeWVersion" readonly
      ...
      <tr>
        <th width="180"><span class="star">*</span>CP Test Program Name</th>
        <td><input name="cpTestProgName" readonly 	class="text_300_protected" type="text"  readonly id="cpTestProgName" value="<%=BeanHelper.getHtmlValueByColumn(ref, "CP_TEST_PROG_NAME") %>"></td>
        </tr>
      <tr>
        <th width="180"><span class="star">*</span>CP Test Program Revision</th>
        <td><input name="cpTestProgRevision" type="text" class="text_protected" readonly id="cpTestProgRevision" value="<%=BeanHelper.getHtmlValueByColumn(ref, "CP_TEST_PROG_REVISION") %>" ></td>
        </tr>
      <tr>
        <th width="180">CP Test Program Release Date</th>
        <td><input name="cpTestProgReleaseDate" type="text" readonly class="text" id="cpTestProgReleaseDate" value="<%=BeanHelper.getHtmlValueByColumn(ref, "CP_TEST_PROG_RELEASE_DATE", "yyyy/MM/dd") %>" >
            <img src="<%=cp%>/images/calendar.gif" alt="Calendar" width="19" height="20" id="cpTestProgReleaseDateBtn">
            <script type="text/javascript">
							Calendar.setup({
	      ...
	      <tr>
	        ...
	        <td><input name="cpCpuTime" type="text" class="text" id="cpCpuTime" value="<%=BeanHelper.getHtmlValueByColumn(ref, "CP_CPU_TIME") %>"></td>
		</tr>
					 
					 <tr>
					   <th width="180"><div class="erp_label">CP Index Time</div></th>
					   <td><input name="cpIndexTime" type="text" class="text" id="cpIndexTime" value="<%=BeanHelper.getHtmlValueByColumn(ref, "CP_INDEX_TIME") %>"></td>
				     </tr>
					 <tr>
					   <th width="180"><div class="erp_label">Contact Die Quantity</div></th>
					   <td><select name="contactDieQty" id="contactDieQty" class="select_w130">
					   	   <option value="">--Select--</option>
                           <option value="1" <%="1".equals(BeanHelper.getHtmlValueByColumn(ref, "CONTACT_DIE_QTY"))?"selected":"" %>>1</option>
                           <option value="1.2" <%="1.2".equals(BeanHelper.getHtmlValueByColumn(ref, "CONTACT_DIE_QTY"))?"selected":"" %>>1.2</option>
                           <option value="2" <%="2".equals(BeanHelper.getHtmlValueByColumn(ref, "CONTACT_DIE_QTY"))?"selected":"" %>>2</option>
                           <option value="2.4" <%="2.4".equals(BeanHelper.getHtmlValueByColumn(ref, "CONTACT_DIE_QTY"))?"selected":"" %>>2.4</option>
                           <option value="3" <%="3".equals(BeanHelper.getHtmlValueByColumn(ref, "CONTACT_DIE_QTY"))?"selected":"" %>>3</option>
                           <option value="4" <%="4".equals(BeanHelper.getHtmlValueByColumn(ref, "CONTACT_DIE_QTY"))?"selected":"" %>>4</option>
						   <option value="8" <%="8".equals(BeanHelper.getHtmlValueByColumn(ref, "CONTACT_DIE_QTY"))?"selected":"" %>>8</option>
                       </select></td>
				     </tr>
					 <tr>
					 ...
					 <tr>
					   ...
					   <td><input name="testerConfig" type="text" class="text" id="testerConfig"  value="<%=BeanHelper.getHtmlValueByColumn(ref, "TESTER_CONFIG") %>"></td>
				     </tr>
					 <tr>
					   <th width="180">1st CP Test House</th>
					   <td><select name="firstCpTestHouse" id="firstCpTestHouse"  class="select_w130">
						        <option value="">--Select--</option>
						        <%
							...
							%>
							<option value="<%=verdorCode %>" <%=verdorCode.equals(BeanHelper.getHtmlValueByColumn(ref, "FIRST_CP_TEST_HOUSE"))?"selected":"" %>><%=to.getShortName() %></option>
							<%
									}
								}
							%>
					 ...
					 <tr>
					   ...
					   <td><input class="text" name="remark" id="remark" value="<%=BeanHelper.getHtmlValueByColumn(ref, "REMARK") %>">&nbsp;
					   </td>
				     </tr>
					 <tr>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0"
				...
				<tr>
					<td height="15"><img src="<%=cp %>/images/spacer.gif"
						...
						<td>
						<input type="hidden" name="materialNum" id="materialNum" value="<%=BeanHelper.getHtmlValueByColumn(ref, "MATERIAL_NUM") %>">
						</td>
				</tr>
			</table>
			</td>
