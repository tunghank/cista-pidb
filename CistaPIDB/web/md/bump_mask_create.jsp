...
function doSave(s) {
	...
	if ($F('maskName') == "") {	
		setMessage("error", "Mask name is must field.");
		return;
	}
	...
	setMessage("error", "Checking bump mask exist...");
	new Ajax.Request(
		'<%=cp%>/ajax/check_bumpMask_exist.do',
		{
			method: 'get',
			parameters: 'maskName='+ $F('maskName') + '&projCode='+ $F('projCode'),
			onComplete: checkBumpMaskExistComplete
		}
	);
}

function checkBumpMaskExistComplete(r) {
	var result = r.responseText;
	if(result=="true") {
		setMessage("error", "Mask name and Project Code is already exist.");
		...
		$('resetBtn').disabled = false;
			...
			$('bumpMaskCreate').action = $('bumpMaskCreate').action + "?m=save&toErp=" + $F('toErp');
			$('bumpMaskCreate').submit();		
	}
}
...
<body>
<form name="bumpMaskCreate" action="<%=cp %>/md/bump_mask_create.do" method="post">
<table width="99%" border="0" cellpadding="0" cellspacing="0">
	<tbody>
		<tr>
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
             <tr>
		          <td class="pageTitle">Master Data :: Create Bumping Mask</td>
             </tr>
           </table>
			...
			<!--
				<tr>
					  ...
					  <td width="30%">
					  	<input type="text" name="prodCode" class="text_protected" id="prodCode" readonly value="<%=BeanHelper.getHtmlValueByColumn(ref, "PROD_CODE") %>">
					  	<img src="<%=cp%>/images/lov.gif" alt="LOV" id="projectcode" onClick="selectProdCode()">					  </td>
  ...
  &nbsp;</td>
			...
			-->
					<tr>
					  ...
					  <td width="30%"><input type="text" name="projCode" class="text_required" id="projCode" readonly value="<%=BeanHelper.getHtmlValueByColumn(ref, "PROJ_CODE") %>">
  ...
  &nbsp;</td>
					...
					<tr>
					  ...
					  <td>
					  	<input class="text_required" type="text" name="maskName" id="maskName">
&nbsp;					  </td>
					  <th>&nbsp;</th>
					  <td>&nbsp;</td>
					</tr>
					<tr>
					  <th><div class="erp_label">RDL</div></th>
					  <td><select id="rdl" name="rdl" class="select_w130">
  ...
  &nbsp; </td>
					...
					<tr>
					  <th>Bump Height </th>
					  <td><input class="text" type="text" name="bumpHeight" id="bumpHeight" value="<%=BeanHelper.getHtmlValueByColumn(ref, "BUMP_HEIGHT") %>">                      </td>
					<th >&nbsp;</th>
					  <td>&nbsp;</td>
					 </tr>
					  <tr>
					...
					<th >&nbsp;</th>
					 ...
					 <tr>
					   <th >Bumping House 1: Mask Ready Date (Plan)</th>
					   <td><input class="text"
					...
					<tr>
					  <th >Bumping House 2: Mask Ready Date (Plan)</th>
					 ...
					 <tr>
					   <th >Bumping House 3: Mask Ready Date (Plan)</th>
					   <td><input class="text" type="text"
					 ...
					 <tr>
					   <th >Bumping House 4: Mask Ready Date (Plan)</th>
					   <td><input class="text" type="text"
					 ...
					 <tr>
					   <th >Bumping House 5: Mask Ready Date (Plan)</th>
					   <td><input class="text" type="text"
