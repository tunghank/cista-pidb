...
<%@ page import="com.cista.pidb.md.to.CpMaterialQueryTo"%>
...
<%
    CpMaterialQueryTo queryTo = (CpMaterialQueryTo) request.getAttribute("queryTo");
	List<CpMaterialTo> result = (List) request.getAttribute("result");
%>
<html>
<head>
<!-- This is header -->
...
<table width="99%" border="0" cellpadding="0" cellspacing="0">
	<tbody>
		<tr>
			...
			<!--Html Start--> <!-- This is header -->
			<jsp:include page="/common/banner.jsp" flush="true" />
			<!-- Content start -->
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
			  <tr>
			    <td class="pageTitle">Master Data :: CP Material Query Result&#13;</td>
			  </tr>
			</table>
			<div class="content">
					...
					<td>
                     ...
                     for (RoleTo roleTo : checkedRoles) 
                     {
					   ...
					   <!-- <input
						...
						%>					  
					  <input	name="button3" type="button" class="button" id="button5"	value="New Query" onClick="document.location.href='<%=cp %>/md/cp_material_query.do?m=pre'">
					  </div>
					</td>
				</tr>
			</table>
			<table class="formFull" id="queryCriteriaPanel" border="0" cellpadding="1" cellspacing="1">
				<tbody>
					<tr>
                     ...
                     <th>CP VARIANT</th>
					  	<td width="30%"><%=BeanHelper.getQueryCriteriaByColumn(queryTo, "CP_VARIANT") %>&nbsp;</td>
					</tr>
					<tr>
         ...
         <%
	      ...
	      {
			%>
			<form name="pagingForm" action="<%=cp %>/md/cp_material_query.do" method="post">
			...
			<table class="formErrorAndButton">
				<tr>
				   ...
				   %>						
						<div align="right">
						    <input name="downloadBtn" type="button" class="button" id="downloadBtn" value="download all" onClick="reportDownload('pagingForm')">
						</div>
					<%
					}
					%>
					...
					<table class="grid" border="0" cellpadding="1" cellspacing="1">
					   ...
					   <tr>
					     ...
					     <th onclick="sort(this, 1)" style="cursor:pointer">Project Code w Version</th>
						  <th onclick="sort(this, 1)" style="cursor:pointer">CP VARIANT</th>
					     <th onclick="sort(this, 1)" style="cursor:pointer">DESCRIPTION</th>
						  <th onclick="sort(this, 1)" style="cursor:pointer">CP TEST PROGRAM NAME LIST</th>
						  <th style="cursor:pointer">REMARK</th>
					   </tr>
		            <%
						int idx = 0;
						for(CpMaterialTo ctp : result) 
						{
							idx ++;
						...
						<tr>
							<td <%=tdcss %>>
							   <input type="radio" name="ref" value="<%=cpMaterialNum%>,<%=projCodeWVersion%>">
							</td>
							<td <%=tdcss %>>
							   <a href="<%=cp%>/md/cp_material_edit.do?m=pre&cpMaterialNum=<%=cpMaterialNum%>&projCodeWVersion=<%=projCodeWVersion%>"><%=BeanHelper.getHtmlValueByColumn(ctp, "CP_MATERIAL_NUM") %>&nbsp;</a>
							</td>
						   <td <%=tdcss %>>
						      <%=BeanHelper.getHtmlValueByColumn(ctp, "PROJECT_CODE_W_VERSION") %>&nbsp;
						   </td>
			            <td <%=tdcss %>>
			               <%=BeanHelper.getHtmlValueByColumn(ctp, "CP_VARIANT") %>&nbsp;
			            </td>
							<td <%=tdcss %>>
							   <%=BeanHelper.getHtmlValueByColumn(ctp, "DESCRIPTION") %>&nbsp;
							</td>
			            <td <%=tdcss %>>
			               <%=BeanHelper.getHtmlValueByColumn(ctp, "CP_TEST_PROGRAM_NAME_LIST") %>&nbsp;
			            </td>
			            <td <%=tdcss %>>
			               <%=BeanHelper.getHtmlValueByColumn(ctp, "REMARK") %>&nbsp;
			            </td>
						</tr>
		            <%
			         }
