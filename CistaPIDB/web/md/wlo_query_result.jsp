...
<%@ page import="com.cista.pidb.md.to.WloQueryTo" %>
...
<script type="text/javascript">
	...
	function createWithRef() {
		if (!$('ref') || getRadioValue('ref') == null) {
			...
			document.location.href='<%=cp%>/md/wlo_create.do?m=pre&ref=' + getRadioValue('ref');
		}
	}
</script>
</head>
<body>
<% 	List<WloTo> result = (List<WloTo>)request.getAttribute("result");
	WloQueryTo criteria = (WloQueryTo)request.getAttribute("criteria");
%>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="pageTitle">Master Data :: WLO Material Query Result</td>
  </tr>
</table>
			<div class="content">
				...
				<tr>
					...
					<div align="right">
					  ...
					  <input name="button1" type="button" class="button" id="button1"value="Create New" 
					 	 onClick="document.location.href='<%=cp%>/md/wlo_create.do?m=pre'">
					  ...
					  onClick="document.location.href='<%=cp%>/md/wlo_query.do?m=pre'">
					</div>
					</td>
				</tr>
			</table>
	...
	<form name="pagingForm" action="<%=cp %>/md/wlo_query.do" method="post">
...
%>
							<tr>
								...
								<td <%=tdcss %>>
								<a href="<%=cp%>/md/wlo_edit.do?m=pre&materialNum=<%=BeanHelper.getHtmlValueByColumn(oneIc, "MATERIAL_NUM") %>"><%=BeanHelper.getHtmlValueByColumn(oneIc, "MATERIAL_NUM")%></a>&nbsp;</td>
