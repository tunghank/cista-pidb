...
<%@ page import="com.cista.pidb.md.to.WaferColorFilterQueryTo"%>
...
<%
    WaferColorFilterQueryTo queryTo = (WaferColorFilterQueryTo) request.getAttribute("queryTo");
	List<WaferColorFilterTo> result = (List) request.getAttribute("result");
%>
<html>
<head>
<!-- This is header -->
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="pageTitle">Master Data ::Wafer Color Filter Query Result&#13;</td>
  </tr>
</table>
			<div class="content">
					...
					<div align="right">
					 ...
					 <!-- <input
					  ...
					  <input
						...
						value="New Query" onClick="document.location.href='<%=cp %>/md/wafer_color_filter_query.do?m=pre'">
					</div>
					</td>
				</tr>
			</table>
			<table class="formFull" id="queryCriteriaPanel" border="0" cellpadding="1" cellspacing="1">
				<tbody>
					<tr>
						<th width="20%">WAFER CF MATERIAL Num.</th>
						<td width="30%"><%=BeanHelper.getQueryCriteriaByColumn(queryTo, "WAFER_CF_MATERIAL_NUM") %>&nbsp;</td>
                          <th>WAFER CF VARIANT</th>
					  	<td width="30%"><%=BeanHelper.getQueryCriteriaByColumn(queryTo, "WAFER_CF_VARIANT") %>&nbsp;</td>
					</tr>
					<tr>
...
%>
<form name="pagingForm" action="<%=cp %>/md/wafer_color_filter_query.do" method="post">
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
    <tr><td>
		<div id="resultPanel" style="overflow:auto;width:100px;height:100px">
					...
					<tr>
					    <th>&nbsp;</th>
						<th onclick="sort(this, 1)" style="cursor:pointer">WAFER CF MATERIAL Num.</th>
					    <th onclick="sort(this, 1)" style="cursor:pointer">Project Code w Version</th>
						<th onclick="sort(this, 1)" style="cursor:pointer">WAFER CF VARIANT</th>
					    <th onclick="sort(this, 1)" style="cursor:pointer">DESCRIPTION</th>
						<th onclick="sort(this, 1)" style="cursor:pointer">REMARK</th>
					</tr>
		 <%
			int idx = 0;
			for(WaferColorFilterTo ctp : result) {
				idx ++;
				String tdcss = "class=\"c" + idx % 2+"\"";				
				String waferCFNum=BeanHelper.getHtmlValueByColumn(ctp, "WAFER_CF_MATERIAL_NUM");
				String projCodeWVersion=BeanHelper.getHtmlValueByColumn(ctp, "PROJECT_CODE_W_VERSION");
		%>				
			<tr>
				<td <%=tdcss %>><input type="radio" name="ref" value="<%=waferCFNum%>,<%=projCodeWVersion%>"></td>
				<td <%=tdcss %>>
				<a href="<%=cp%>/md/wafer_color_filter_edit.do?m=pre&waferCfMaterialNum=<%=waferCFNum%>&projCodeWVersion=<%=projCodeWVersion%>"><%=BeanHelper.getHtmlValueByColumn(ctp, "WAFER_CF_MATERIAL_NUM") %>&nbsp;</a></td>
			    <td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(ctp, "PROJECT_CODE_W_VERSION") %>&nbsp;</td>
                <td <%=tdcss %>><%=BeanHelper.getHtmlValueByColumn(ctp, "WAFER_CF_VARIANT") %>&nbsp;</td>
