...
<%
	ZsSdramTo ref = (ZsSdramTo) request.getAttribute("ref");
...
function doSave(s) {
	...
	setMessage("error", "Checking ZS_SDRAM exist...");
	new Ajax.Request(
		'<%=cp%>/ajax/check_zsSdram_exist.do',
		{
			method: 'get',
			parameters: 'materialNum='+ $F('materialNum') + '&description=' + $F('description'),
			onComplete: checkZsSdramExistComplete
		}
	);
}

function checkZsSdramExistComplete(r) {
	var result = r.responseText;
	if(result == "materialNum") {
		setMessage("error", "materialNum is error.");
		$('releaseBtn').disabled = false;
		$('saveBtn').disabled = false;
		$('resetBtn').disabled = false;
	} else if(result == "description") {
		setMessage("error", "description is error.");
		$('releaseBtn').disabled = false;
		$('saveBtn').disabled = false;
		$('resetBtn').disabled = false;
	}else {
		...
		if (incompleted != "") {
			if (confirm("The following required fields is incomplete, the SDRAM will be saved as draft:\r\n" + incompleted.substring(2))) {
				setMessage("error", "Saving SDRAM...");
				selectAllOptions($('sdramVendor'));
				selectAllOptions($('prodCodeList'));
				$('zsSdramForm').action = $('zsSdramForm').action + "&toErp=" + $F('toErp');
				$('zsSdramForm').submit();
			} else {
				$('releaseBtn').disabled = false;
				$('saveBtn').disabled = false;
			...
			setMessage("error", "Saving ZS-SDRAM...");
			selectAllOptions($('sdramVendor'));
			 selectAllOptions($('prodCodeList'));
			$('zsSdramForm').action = $('zsSdramForm').action + "&toErp=" + $F('toErp');
			$('zsSdramForm').submit();
...
function selectVendorComplete(selectedTests) {
	if (selectedTests && selectedTests.length>0) {
		for(var i=0; i<selectedTests.length; i++) {
			...
			addOption($('sdramVendor'), test[1], test[0]);
		}
	}
}
...
<form name="ZsSdramForm" action="<%=cp %>/md/zs_sdram_create.do?m=save" method="post">
	...
	<tbody>
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="pageTitle">Master Data :: Create ZS_SDRAM</td>
				</tr>
			</table>
			<div class="content">
			...
			<table class="formFull" border="0" cellpadding="1" cellspacing="1">
				<tbody>
					...
					<tr>
						<th width="20%" rowspan="2" style="vertical-align:top"><div>SDRAM Vendor</div></th>
					  	<td width="30%" rowspan="2" style="vertical-align:top">
					  	<input type="button" class="button" value="Add" onclick="selectVendor()"><input type="button" class="button" value="Remove" onclick="removeSelectedOptions($('sdramVendor'))"><br>
					  	<select size="4" multiple style="width:50%" name="sdramVendor" id="sdramVendor">
					    	<%
						...
						<th width="20%" rowspan="3" style="vertical-align:top">Remark</th>
						<td width="30%" rowspan="3" style="vertical-align:top"><textarea id="remark" name="remark" cols="40" rows="6" class="text"><%=BeanHelper.getHtmlValueByColumn(ref, "REMARK") %></textarea></td>
					</tr>	
					<tr>						
					</tr>
