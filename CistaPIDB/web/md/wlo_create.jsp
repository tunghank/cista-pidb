...
<%@ page import="com.cista.pidb.md.to.WloTo"%>
...
function doSave() {
		...
		/*if ($F('golCoating') == "") {
			setMessage("error", "GOL Coating is must field.");
			$('isRelease').value='0';	
			return;
		}*/
		...
		new Ajax.Request(
			'<%=cp%>/ajax/check_wlo_exist.do',
			{
				method: 'get',
				parameters: 'materialNum='+ $F('materialNum'),
				onComplete: checkWloExistComplete
			}
		);
	}	
	
 function checkWloExistComplete(r) {
	var result = r.responseText;
	if(result=="true") {
		setMessage("error", "The WLO Material is already exist.");
		...
		$('isRelease').value='0';	
			...
			setMessage("error", "Saving WLO Material package...");		
...
function fetchWloMaterialNum() {
	new Ajax.Request(
			'<%=cp%>/ajax/fetch_WloMaterialNum.do',
			{
				method: 'post',
				...
				onComplete: fetchWloMaterialNumComplete
			}
		);
}

function fetchWloMaterialNumComplete(r) {
	var result = r.responseText;
	var rt  = result.split("|");
	var material = rt[0];
	var partNum = rt[1];
	...
	if (result == "") {
	  	setMessage("error", "Can not fetch WLO Material Number.");		  	
	} else {
		$('materialNum').value = material;
		$('partNum').value = partNum;
	}
	$('prodCode').value =  $F('prodName') ;
}
...
<body>
<%
	WloTo ref = (WloTo) request.getAttribute("ref");
%>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
		<td class="pageTitle">Master Data :: Create WLO Material</td>
  </tr>
</table>
			<div class="content">
			<form id="createForm" action="<%=cp%>/md/wlo_create.do" method="post">
			...
			<table class="formFull" border="0" cellpadding="1" cellspacing="1">
				<tbody>
					...
					<tr>
					    ...
					    <td width="30%"><select class="select_w130" name="prodType" id="prodType" onChange="fetchWloMaterialNum()">
								<option value="">--Select--</option>
								<%
						...
						<th width="20%">Thickness of Glue</th>
					    <td width="30%">
						  <select class="select_w130" name="thicknessOfGlue" id="thicknessOfGlue">
								<option value="">--Select--</option>
								<%
										List<FunctionParameterTo> thicknessOfGlueList = (List<FunctionParameterTo>) request.getAttribute("thicknessOfGlueList");
										for (FunctionParameterTo version : thicknessOfGlueList) {
											String v = version.getFieldValue();
											String selected = "";
											if (v.equals(BeanHelper.getHtmlValueByColumn(ref,"THICKNESS_OF_GLUE")))
											{
												selected = "selected";
											}
								%>
					...
					<tr>
					    ...
					    <td width="30%"><select class="select_w130" name="vendorCode" id="vendorCode" onChange="fetchWloMaterialNum()">
								<option value="">--Select--</option>
								<%
					    ...
					    <td width="30%">
						  <select class="select_w130" name="packingType" id="packingType" onChange="fetchWloMaterialNum()">
								<option value="">--Select--</option>
								<%
					...
					<tr>
					    ...
					    <td width="30%"><select class="select_w130" name="serialNum" id="serialNum" onChange="fetchWloMaterialNum()">
								<option value="">--Select--</option>
								<%
					...
					<tr>
					    ...
					    <td width="30%"><input class="text" type="text" name="variant" id="variant" 
						 size='1' maxlength="2" onChange="fetchWloMaterialNum()" onBlur="fetchWloMaterialNum()"></td>
					...
					<tr>
						...
						value="0" onChange="fetchWloMaterialNum()" onBlur="fetchWloMaterialNum()" size='1' maxlength="1"></td>
						<th width="20%">&nbsp;</th>
					  	<td width="30%">&nbsp;</td>
					</tr>
