...
function doSave() {
	...
	if ($F('productLine') == "") {	
		setMessage("error", "Product Line is must field.");
		return;
	}
	if ($F('productFamily') == "") {	
		setMessage("error", "Product Family is must field.");
		return;
	}
	if ($F('cpIndexTimeE') == "") {	
	...
	setMessage("error", "Checking product standard test reference exist...");
	new Ajax.Request(
		'<%=cp%>/ajax/check_prodStdTestRef_exist.do',
		{
			method: 'get',
			parameters: 'productLine='+ $F('productLine') + '&productFamily=' + $F('productFamily'),
			onComplete: checkprodStdTestRefExistComplete
		}
	);
}

function checkprodStdTestRefExistComplete(r) {
	var result = r.responseText;
	if(result=="true") {
		setMessage("error", "There is already Test Reference ID with same Product Line,Product Family and Variant.");
		...
		$('resetBtn').disabled = false;
			...
			$('prodStdTestRefCreate').action = $('prodStdTestRefCreate').action + "?m=save";
			$('prodStdTestRefCreate').submit();
	}
}
...
<body>
<form name="prodStdTestRefCreate"
	action="<%=cp %>/md/prod_std_test_ref_create.do" method="post">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="pageTitle">Master Data :: Create Product Standard Test Reference </td>
  </tr>
</table>
			<div class="content">
				...
				<tbody>
					<tr>
						<th width="180"><div class="erp_label"><span class="star">*</span>Test Reference ID</div></th>
					  	<td><input class="text_protected" readonly type="text" id="testReferenceId"></td>
					</tr>
					<tr>
					  <th><span class="star">*</span> Variant</th>
					  <td><input class="text_protected" readonly type="text" id="variant"></td>
					</tr>
					<tr>
						<th><div class="erp_label"><span class="star">*</span>Product Line</div></th>
                          <td><select id="productLine" name="productLine" class="select_w130_required">
                            <option value="">--Select--</option>
					        <option value="OA" <%="OA".equals(BeanHelper.getHtmlValueByColumn(ref, "PRODUCT_LINE"))?"selected":"" %>>OA</option>
					        <option value="AV" <%="AV".equals(BeanHelper.getHtmlValueByColumn(ref, "PRODUCT_LINE"))?"selected":"" %>>AV</option>
					        <option value="Mobile" <%="Mobile".equals(BeanHelper.getHtmlValueByColumn(ref, "PRODUCT_LINE"))?"selected":"" %>>Mobile</option>
					        <option value="Video" <%="Video".equals(BeanHelper.getHtmlValueByColumn(ref, "PRODUCT_LINE"))?"selected":"" %>>Video</option>
					        <option value="ISP" <%="ISP".equals(BeanHelper.getHtmlValueByColumn(ref, "PRODUCT_LINE"))?"selected":"" %>>ISP</option>
					        <option value="Power" <%="Power".equals(BeanHelper.getHtmlValueByColumn(ref, "PRODUCT_LINE"))?"selected":"" %>>Power</option>
					        <option value="OP" <%="OP".equals(BeanHelper.getHtmlValueByColumn(ref, "PRODUCT_LINE"))?"selected":"" %>>OP</option>
					        <option value="Vcom" <%="Vcom".equals(BeanHelper.getHtmlValueByColumn(ref, "PRODUCT_LINE"))?"selected":"" %>>Vcom</option>
					        <option value="PMP" <%="PMP".equals(BeanHelper.getHtmlValueByColumn(ref, "PRODUCT_LINE"))?"selected":"" %>>PMP</option>
					        <option value="IP" <%="IP".equals(BeanHelper.getHtmlValueByColumn(ref, "PRODUCT_LINE"))?"selected":"" %>>IP</option>
					        <option value="Others" <%="Others".equals(BeanHelper.getHtmlValueByColumn(ref, "PRODUCT_LINE"))?"selected":"" %>>Others</option>
					  	</select></td>
					</tr>
					<tr>
					  <th><div class="erp_label"><span class="star">*</span>Product Family</div></th>
					  <td><select id="productFamily" name="productFamily" class="select_w130_required">
                          <option value="">--Select--</option>
                          <%
							...
							%>
                          <option value="<%=option.getProductFamily() %>" <%=(option.getProductFamily()).equals(BeanHelper.getHtmlValueByColumn(ref, "PRODUCT_FAMILY"))?"selected":"" %>><%=option.getDescription() %></option>
                          <%
								}
							%>
                      </select></td>
