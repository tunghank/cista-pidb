...
public class ZsUsbramDao extends PIDBDaoSupport {

	public List<ZsUsbramTo> findBySearchPage(final ZsUsbramQueryTo findInstance) {
		SimpleJdbcTemplate stj = super.getSimpleJdbcTemplate();
		String sql = "select * from PIDB_USB_RAM where 1=1";
		...
		if (findInstance.getPageNo() > 0) {
			...
			return stj.query(getPagingSql(sql, cursorFrom, cursorTo),
					new GenericRowMapper<ZsUsbramTo>(ZsUsbramTo.class),
					new Object[] {});
		} else {
			return stj.query(sql, new GenericRowMapper<ZsUsbramTo>(
					ZsUsbramTo.class), new Object[] {});
		}

	}

	public List<ZsUsbramTo> findAll() {
		SimpleJdbcTemplate stj = super.getSimpleJdbcTemplate();
		String sql = "select * from PIDB_USB_RAM";
		logger.debug(sql);
	...
	public ZsUsbramTo findData(String materialNum, String description) {
		SimpleJdbcTemplate stj = super.getSimpleJdbcTemplate();
		String sql = "select * from PIDB_USB_RAM Where MATERIAL_NUM=? AND DESCRIPTION=?";
		logger.debug(sql);
		List<ZsUsbramTo> result = stj.query(sql,
				new GenericRowMapper<ZsUsbramTo>(ZsUsbramTo.class),
	...
	public ZsUsbramTo findByPrimaryKey(final String _meterialNum) {
		SimpleJdbcTemplate stj = super.getSimpleJdbcTemplate();
		String sql = "select * from PIDB_USB_RAM where MATERIAL_NUM = ?";
		logger.debug(sql);
		List<ZsUsbramTo> result = stj.query(sql,
				new GenericRowMapper<ZsUsbramTo>(ZsUsbramTo.class),
	 ...
	 * @param newInstance
	 */
	public void insertUsbram(final ZsUsbramTo newInstance) {
		if (null == newInstance) {
			logger.error("Error the new PIDB_USB_RAM object is null.");
		}
			...
			protected void doInTransactionWithoutResult(
				...
				sjt.update(sql, newInstance.getMaterialNum(), newInstance
						...
						newInstance.getModifiedBy(), newInstance
								.getVendorCode(), newInstance.getIo(),
						newInstance.getMaxResolution(), newInstance
								.getReleaseTo());
	...
	public void deleteByPrimaryKey(final String meterialNum) {
		TransactionCallbackWithoutResult callback = new TransactionCallbackWithoutResult() {
			public void doInTransactionWithoutResult(
					final TransactionStatus status) {
				SimpleJdbcTemplate sjt = getSimpleJdbcTemplate();
				String sql = "delete from PIDB_USB_RAM where MATERIAL_NUM =? ";
				logger.debug(sql);
	...
	public void updateUsbram(final ZsUsbramTo newInstance) {
		if (newInstance == null) {
		...
		TransactionCallbackWithoutResult callback = new TransactionCallbackWithoutResult() {
			protected void doInTransactionWithoutResult(
					final TransactionStatus status) {
				SimpleJdbcTemplate sjt = getSimpleJdbcTemplate();
				String sql = "update PIDB_USB_RAM set " + "MATERIAL_NUM=?,"
						+ "DESCRIPTION=?," + "MEMORY_SIZE=?,"
						+ "OPERATION_VOLTAGE=?," + "THICKNESS=?,"
						+ "APPLICATION_PRODUCT=?," + "WAFER_INCH=?,"
						+ "REMARK=?," + "GROSS_DIE=?," + "CREATED_BY=?,"
						+ "MODIFIED_BY=?," + "VENDOR_CODE=?," + "IO=?,"
						+ "MAX_RESOLUTION=? , RELEASE_TO=? "
						+ "where MATERIAL_NUM=? ";
				logger.debug(sql);
				sjt.update(sql, newInstance.getMaterialNum(), newInstance
						...
						newInstance.getModifiedBy(), newInstance
								.getVendorCode(), newInstance.getIo(),
						newInstance.getMaxResolution(), newInstance
								.getReleaseTo(), newInstance.getMaterialNum());
			}
		};
		doInTransaction(callback);
	}

	public int countResult(ZsUsbramQueryTo zsUsbramQueryTo) {
		SimpleJdbcTemplate sjt = getSimpleJdbcTemplate();
		String sql = "select count(*) from PIDB_USB_RAM where 1=1";
		if (zsUsbramQueryTo.getReleaseTo() != null
				&& !zsUsbramQueryTo.getReleaseTo().equals("")) {
			if (zsUsbramQueryTo.getReleaseTo().equals("HX")
					|| zsUsbramQueryTo.getReleaseTo().equals("WP")) {
				sql += " AND (RELEASE_TO ='" + zsUsbramQueryTo.getReleaseTo() + "'"
						+ " or RELEASE_TO ='ALL')";
			}
		}
		String where = generateWhereCause(zsUsbramQueryTo);
		sql = sql + where;
		return sjt.queryForInt(sql, new Object[] {});
	}

	private String generateWhereCause(final ZsUsbramQueryTo queryTo) {
		...
		if (queryTo.getIo() != null && !queryTo.getIo().equals("")) {
			String ioString = getSmartSearchQueryString("IO", queryTo.getIo());
			if (ioString != null) {
				sb.append(" and (" + ioString + " )");
			}
		}
		if (queryTo.getMaxResolution() != null
				&& !queryTo.getMaxResolution().equals("")) {
			String maxResolutionString = getSmartSearchQueryString(
					"MAX_RESOLUTION", queryTo.getMaxResolution());
			if (maxResolutionString != null) {
				sb.append(" and (" + maxResolutionString + " )");
			}
		}
	...
	public List<String> findMaterialNum() {
		...
		String sql = "select distinct MATERIAL_NUM from PIDB_USB_RAM order by MATERIAL_NUM";
		logger.debug(sql);
